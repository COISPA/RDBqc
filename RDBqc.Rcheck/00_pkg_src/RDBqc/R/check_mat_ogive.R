...
#' @export
#' @examples check_mat_ogive(data_ex,species="Mullus barbatus",
#' sex="F",immature_stages=c("0","1","2a"))
#' @importFrom graphics legend lines
#' @importFrom stats binomial coefficients glm predict

check_mat_ogive<-function(data,species,sex,immature_stages=c("0","1","2a"),verbose=TRUE){
...
plot(merge_temp$Length_class, merge_temp$Mature/merge_temp$Total,cex=1.5,xlab="length(mm)",ylab="proportion of matures",main=paste(species," - Females - ",years,sep=""))
...
plot(merge_temp$Length_class, merge_temp$Mature/merge_temp$Total,cex=1.5,xlab="length(mm)",ylab="proportion of matures",main=paste(species," - Males - ",years,sep=""))
