...
#' @export EWG_plot_discard_ts
#' @examples EWG_plot_discard_ts(disc=discards,MS="ITA",GSA=11,SP="ARA",by="quarter")
...
EWG_plot_discard_ts <- function(disc,MS,GSA,SP,by="year"){
    ...
    if (FALSE) {
        ...
        verbose=TRUE
        disc=discards
        EWG_plot_discard_ts(disc=discards,MS="ITA",GSA=18,SP="ARA",by="year")
    }
  ...
  if (nrow(disc) > 0) {
    ...
    if (by=="quarter"){
        ...
        plot=
            ggplot(tmpdisc,aes(x=year,y=tot,fill=gear))+geom_area(colour="black", size=.2, alpha=.8)+theme_bw() +scale_x_continuous(breaks=seq(min(disc_tmp$year),max(gear_tmp$year),2)) +
