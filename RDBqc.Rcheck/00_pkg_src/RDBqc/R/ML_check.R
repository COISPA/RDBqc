...
ML_tab_check<-function(ML_tab,SP,MS,GSA) {
...
ML_tab[ML_tab$SEX==FALSE,]$SEX="F"
    ...
    for (i in unique(as.character(ML_tab$SEX))){
        ggplot(data=ML_tab[ML_tab$SEX%in% i,], aes(x=LENGTHCLASS,y= PRM,col=factor(START_YEAR))) + geom_line(stat="identity") +theme(legend.position = "bottom") +expand_limits(x = 0, y = 0)+theme(legend.text = element_text(color = "blue", size = 6))+guides(col=guide_legend(title=paste(SP,GSA,MS))
