...
ggplot(VBGF,aes(x=AGE,y=LENGTH,col=SEX))+geom_point()+geom_line()+
...
for (i in unique(VBGF$SEX)){
    ggplot(VBGF[VBGF$SEX%in%i,],aes(x=AGE,y=LENGTH,col=ID))+geom_point()+geom_line()+
...
for (i in unique(VBGF$SEX)){
    ggplot(VBGF[VBGF$SEX%in%i,],aes(x=AGE,y=LENGTH,col=ID))+geom_point()+geom_line()+ggtitle(paste0("VBGF curve of ",i," ",SP, " in ", MS,"_GSA",GSA))+theme(legend.position = "bottom")+theme(
        legend.text = element_text(color = "blue", size = 6))+guides(col=guide_legend(title="")
        )
    }
