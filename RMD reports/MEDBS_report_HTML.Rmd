...
Catch <- Catch_tab_example #read.coded.file("./catch.csv", sep = ",")
Land <- Landing_tab_example #read.coded.file("./landings.csv", sep = ",")
Disc <- Discard_tab_example #read.coded.file("./discards.csv", sep = ",")
ML <-ML_tab_example # read.coded.file("./ml.csv", sep = ",")
MA <- MA_tab_example #read.coded.file("./ma.csv", sep = ",")
SL <- SL_tab_example #read.coded.file("./srl.csv", sep = ",")
SA <- SA_tab_example #read.coded.file("./sra.csv", sep = ",")
GP <- GP_tab_example #read.coded.file("./gp.csv", sep = ",")
ALK <- ALK_tab_example #read.coded.file("./alk.csv", sep = ",")
  ...
  for (g in 1:length(GSAs)) {
    ...
    for (s in 1:length(SPs)) {
      ...
      if (class(res) == "list" & length(res) > 0) {
        ...
        for (l in 1:length(res)) {
          sex <- strsplit(names(res[l]), " - ")[[1]][1]
          cat(paste0("\nPlot of mean length at age from ALK for ", SPs[s], " - ", MS, " - ", GSAs[g], " - ", sex, "\n"))
          print(res[[l]])
          cat("\n\n\n")
        }
      } else {
