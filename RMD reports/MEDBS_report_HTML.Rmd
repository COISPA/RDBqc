...
if (check_Catch) {
  ...
  for (g in 1:length(GSAs)) {
    ...
    for (s in 1:length(SPs)) {
      ...
      if (!is.null(res)) {
        if (length(res)>0) {
           cat(paste0("\n- The following years are missing in the Catch data time series (", SPs[s], " - ", MS, " - ", GSAs[g], "): ",paste(res[[1]],collapse=", "),"\n")) 
            
            if (!is.null(res[2])) {
          cat(paste0("\n_All the combination of country and areas detected in data are reported in the following table_:"))
          print(res[[2]] %>% kable("html") %>% kable_styling(font_size = 12) %>% kable_classic(full_width = TRUE))
          cat("\n\n\n")
        } else {
          cat("\n\n\n- No data data available for the selected country")
        }   
...
if (check_Land) {
  ...
  for (g in 1:length(GSAs)) {
    ...
    for (s in 1:length(SPs)) {
      ...
      if (!is.null(res)) {
        if (length(res)>0) {
           cat(paste0("\n- The following years are missing in the landings data time series (", SPs[s], " - ", MS, " - ", GSAs[g], "): ",paste(res[[1]],collapse=", "),"\n")) 
...
if (check_Disc) {
  ...
  for (g in 1:length(GSAs)) {
    ...
    for (s in 1:length(SPs)) {
      ...
      if (!is.null(res)) {
        if (length(res)>0) {
           cat(paste0("\n- The following years are missing in the discards data time series (", SPs[s], " - ", MS, " - ", GSAs[g], "): ",paste(res[[1]],collapse=", "),"\n")) 
