...
if (exists("Catch")) {
  if (class(Catch)=="data.frame") {
    if(nrow(Catch)>0){
      ...
      cat(paste0("\n# Catch table\n"))
      if(!user_SP){
        SPs <- unique(Catch$species)
      }
      if(!user_MS){
        MS  <- unique(Catch$country)
      }
      if(!user_GSA){
        GSAs <- unique(Catch$area)
      }
    }
  }
} else {
...
if (exists("Land")) {
  if (class(Land)=="data.frame") {
    if(nrow(Land)>0){
      ...
      cat(paste0("\n# Landing table\n"))
      if(!user_SP){
        SPs <- unique(Land$species)
      }
      if(!user_MS){
        MS  <- unique(Land$country)
      }
      if(!user_GSA){
        GSAs <- unique(Land$area)
      }
    }
  }
} else {
...
```{r KS_test, results = 'asis', fig.height=6, fig.width=9}
    ...
    cat(paste0("\n### ", GSAs[g], "\n"))
      ...
      for (s in 1:length(SPs)) {
        ...
        if (class(res)== "list" & length(res)>0){
            ...
            tab1 <- res[[2]]
            tab1 <- tab1[tab1$Comment =="belong to same population",]
...
```{r lengthclass_0, results = 'asis', fig.height=6, fig.width=9}
if (check_Land) {
    cat("\n## Checks the presence of length classes numbers with zeros in landings\n")
...
```{r length_ind, results = 'asis', fig.height=6, fig.width=9}
if (check_Land) {
...
```{r yr_missing_length, results = 'asis', fig.height=6, fig.width=9}
if (check_Land) {
    cat("\n## Checks the presence of years with missing length distribution\n")
...
```{r weight_0, results = 'asis', fig.height=6, fig.width=9}
if (check_Land) {
    cat("\n## Checks the presence of lending weights with zeros in landings\n")
...
```{r weight_minus1, results = 'asis', fig.height=6, fig.width=9}
if (check_Land) {
    cat("\n## Checks the presence of landing weights with -1 in landings\n")
...
```{r land_mean_weight, results = 'asis', fig.height=6, fig.width=9}
if (check_Land) {
...
if (check_Land) {
    ...
    cat(paste0("\nTo identify possible inconsistencies in landing data, the species time series of landing volumes is here plottet by quarter.\n"))
...
if (check_Land) {
    ...
    cat(paste0("\nTo identify possible inconsistencies in landing data, the species time series of landing volumes is here plottet by fishery and gear.\n"))
      ...
      for (s in 1:length(SPs)) {
        ...
        if (class(res)== "list" & length(res)>0){
            ...
            print(res[[1]] %>% kable("html") %>% kable_styling(font_size = 12) %>% kable_classic(full_width=TRUE) )
               ...
               cat(paste0("\nPlot of sex ratio at age for ",SPs[s]," - ",MS," - ",GSAs[g], " - ", text, "\n"))
               print(res[[2]])
               cat("\n\n\n")
               # 
               #                cat(paste0("\nPlot of sex ratio at age by year for ",SPs[s]," - ",MS," - ",GSAs[g], " - ", text, "\n"))
               # print(res[[3]])
               # cat("\n\n\n")
      ...
      for (s in 1:length(SPs)) {
        ...
        if (class(res)== "list" & length(res)>0){
            ...
            print(res[[1]] %>% kable("html") %>% kable_styling(font_size = 12) %>% kable_classic(full_width=TRUE) )
               ...
               cat(paste0("\nPlot of sex ratio at length for ",SPs[s]," - ",MS," - ",GSAs[g], " - ", text, "\n"))
               print(res[[2]])
               cat("\n\n\n")
               # 
               #                cat(paste0("\nPlot of sex ratio at age by year for ",SPs[s]," - ",MS," - ",GSAs[g], " - ", text, "\n"))
