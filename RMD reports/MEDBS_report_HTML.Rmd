...
lib <- c("RDBqc", "knitr", "kableExtra", "dplyr", "ggplot2", "rworldmap", "sp", "rworldxtra", "pander", "data.table", "grDevices", "magrittr", "tictoc", "tidyverse", "fishmethods", "tidyr", "gridExtra","readr","ggpubr")
...
MS <- "ROU"
GSAs <- NA # sort(c("GSA 20","GSA 22","GSA 23"))
end_year <- 2022
```
...
```{r catch_LW, results = 'asis',fig.height=6,fig.width=9}
if (check_Catch) {
    ...
    for (s in 1:length(SPs)) {
      ...
      res <- suppressMessages(MEDBS_Catch_LW(data=Catch, SP = SPs[s], MS = MS, GSA = GSAs[g], verbose = FALSE))
       ...
       if (class(res)[1] == "NULL") {
         ...
         print(res)
           #cat(paste0("L-W scatter plots produced for ", SPs[s], " in ", GSAs[g], "_\n\n"))
       }
    ...
    for (s in 1:length(SPs)) {
      ...
      res <- suppressMessages(MEDBS_Catch_CAA(data=Catch, SP = SPs[s], MS = MS, GSA = GSAs[g], verbose = FALSE))
       ...
       if (class(res)[1] == "NULL") {
          ...
          #cat(paste0("Check on numbers by age in landing and discard for ", SPs[s], " in ", GSAs[g], "executed _\n\n"))
        print(res)
        }
