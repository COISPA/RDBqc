...
if (check_Catch) {
  ...
  for (g in 1:length(GSAs)) {
    s <- 1
    for (s in 1:length(SPs)) {
      res <- MEDBS_check_missing_years(data = Catch, type = "c", SP = SPs[s], MS = MS, GSA = GSAs[g], verbose = FALSE)
      if (!is.null(res)) {
        if (length(res)>0) {
           cat(paste0("\n- The following years were detected in the Catch data time series (", SPs[s], " - ", MS, " - ", GSAs[g], "): ",paste(res,collapse=", "),"\n")) 
        } else {
...
if (check_Land) {
  ...
  for (g in 1:length(GSAs)) {
    s <- 1
    for (s in 1:length(SPs)) {
      res <- MEDBS_check_missing_years(data = Land, type = "l", SP = SPs[s], MS = MS, GSA = GSAs[g], verbose = FALSE)
      if (!is.null(res)) {
        if (length(res)>0) {
           cat(paste0("\n- The following years were detected in the landings data time series (", SPs[s], " - ", MS, " - ", GSAs[g], "): ",paste(res,collapse=", "),"\n")) 
        } else {
  ...
  for (g in 1:length(GSAs)) {
    ...
    for (s in 1:length(SPs)) {
      ...
      if (!is.null(res) & length(res)!=0) {
          ...
          if (!is.null(res[[1]]) & length(res)>1) {
              if (length(res[[2]]>0)) {
               cat(paste0("\n- No length distributions available for the following years: ", paste(res[[2]],collapse = ", "), "\n"))
              }
          } else {
...
if (check_Disc) {
  cat("\n## No replocated records in discard data \n")
...
if (check_Disc) {
  ...
  for (g in 1:length(GSAs)) {
    s <- 1
    for (s in 1:length(SPs)) {
      res <- MEDBS_check_missing_years(data = Disc, type = "d", SP = SPs[s], MS = MS, GSA = GSAs[g], verbose = FALSE)
      if (!is.null(res)) {
        if (length(res)>0) {
           cat(paste0("\n- The following years were detected in the discards data time series (", SPs[s], " - ", MS, " - ", GSAs[g], "): ",paste(res,collapse=", "),"\n")) 
        } else {
  ...
  for (g in 1:length(GSAs)) {
    ...
    for (s in 1:length(SPs)) {
      ...
      if (!is.null(res) & length(res)!=0) {
          ...
          if (!is.null(res[[1]])) {
              if (length(res[[2]]>0) & length(res)>1) {
               cat(paste0("\n- No length distributions available for the following years: ", paste(res[[2]],collapse = ", "), "\n"))
              }  else {
                  cat(paste0("\n- No length distributions available in discards for the selected combination: ", SPs[s], " - ", MS, " - ", GSAs[g], ".\n"))
          }
          }
        ...
        cat(paste0("\n- No years in discards with missing length distribution for : ", SPs[s], " - ", MS, " - ", GSAs[g], "\n"))
      }
    }
  }
} # check_Disc
