...
lib <- c("RDBqc", "knitr", "kableExtra", "dplyr", "ggplot2", "rworldmap", "sp", "rworldxtra", "pander", "data.table", "grDevices", "magrittr", "tictoc", "tidyverse", "fishmethods", "tidyr", "gridExtra","readr","ggpubr","HH","lattice","stringr")
...
Catch <- read.table("D:/OneDrive - Coispa Tecnologia & Ricerca S.C.A.R.L/RDB3/test RDBqc/commercial 3/ROU/ROU_catch.csv", sep = ",", header = TRUE)
Land <- read.table("D:/OneDrive - Coispa Tecnologia & Ricerca S.C.A.R.L/RDB3/test RDBqc/commercial 3/ROU/ROU_landings.csv", sep = ",", header = TRUE)
Disc <- read.table("D:/OneDrive - Coispa Tecnologia & Ricerca S.C.A.R.L/RDB3/test RDBqc/commercial 3/ROU/ROU_discards.csv", sep = ",", header = TRUE)
ML <- read.table("D:/OneDrive - Coispa Tecnologia & Ricerca S.C.A.R.L/RDB3/test RDBqc/commercial 3/ROU/ROU_ml.csv", sep = ",", header = TRUE)
MA <- read.table("D:/OneDrive - Coispa Tecnologia & Ricerca S.C.A.R.L/RDB3/test RDBqc/commercial 3/ROU/ROU_ma.csv", sep = ",", header = TRUE)
SL <- read.table("D:/OneDrive - Coispa Tecnologia & Ricerca S.C.A.R.L/RDB3/test RDBqc/commercial 3/ROU/ROU_srl.csv", sep = ",", header = TRUE)
SA <- read.table("D:/OneDrive - Coispa Tecnologia & Ricerca S.C.A.R.L/RDB3/test RDBqc/commercial 3/ROU/ROU_sra.csv", sep = ",", header = TRUE)
GP <- read.table("D:/OneDrive - Coispa Tecnologia & Ricerca S.C.A.R.L/RDB3/test RDBqc/commercial 3/ROU/ROU_gp.csv", sep = ",", header = TRUE)
ALK <- read.table("D:/OneDrive - Coispa Tecnologia & Ricerca S.C.A.R.L/RDB3/test RDBqc/commercial 3/ROU/ROU_alk.csv", sep = ",", header = TRUE)

SPs <- NA # sort(c("HKE","MUT"))
MS <- "ROU"
GSAs <- NA # sort(c("GSA 29"))


end_year <- 2024

RDBFIS_embedded_version_analysis <- TRUE
...
# ---- FIGURE PROFILES ----
if (isTRUE(RDBFIS_embedded_version_analysis)) {
  .fig_w <- 8   # o quello che vuoi in embedded
  .fig_h <- 5
  .fig_dpi <- 72
} else {
  .fig_w <- 9
...
print_tbl_fast <- function(x, n = Inf, font_size = 11) {
  x <- as.data.frame(x)
  if (is.finite(n) && nrow(x) > n) x <- x[1:n, , drop = FALSE]
...
if (exists("Catch")) {
  if (is.data.frame(Catch)) {
    if (nrow(Catch) > 0) {
      ...
      if (!user_SP)  SPs  <- sort(unique(Catch$species))
      if (!user_MS)  MS   <- unique(Catch$country)
      if (!user_GSA) GSAs <- sort(unique(Catch$area))

      Catch <- Catch[Catch$country %in% MS, ]
      Catch <- Catch[Catch$area    %in% GSAs, ]
      Catch <- Catch[Catch$species %in% SPs, ]

      if (nrow(Catch) == 0) {
        stop("Catch has no data after filtering (0 rows). Check MS/GSAs/SPs selections.")
      }
    ...
    cat("\n Catch exists but is not a data.frame \n")
  }
} else {
  cat("\n No catch data available \n")
}
  ...
  for (g in 1:length(GSAs)) {
    ...
    for (s in 1:length(SPs)) {
      ...
      if (!is.null(res)) {
        ...
        if (!RDBFIS_embedded_version_analysis){
  ...
  for (g in 1:length(GSAs)) {
    ...
    for (s in 1:length(SPs)) {
      ...
      if (nrow(res) > 0) {
        ...
        print(print_tbl_fast(res, n = Inf, font_size = 12))
...
if (check_Catch) {
  ...
  for (g in 1:length(GSAs)) {
    ...
    for (s in 1:length(SPs)) {
      ...
      res <- suppressMessages(MEDBS_Catch_LW(data=Catch, SP = SPs[s], MS = MS, GSA = GSAs[g], verbose = FALSE))
      
      
       if (class(res)[1] == "NULL") {
         cat(paste0("Few or null data on length and weight for ", SPs[s], " in ", GSAs[g], "_\n\n"))
  ...
  for (g in 1:length(GSAs)) {
    ...
    for (s in 1:length(SPs)) {
      ...
      res <- suppressMessages(MEDBS_Catch_CAA(data=Catch, SP = SPs[s], MS = MS, GSA = GSAs[g], verbose = FALSE))
        ...
        res <- suppressMessages(MEDBS_Catch_CAA(data=Catch, SP = SPs[s], MS = MS, GSA = GSAs[g], verbose = FALSE,embedded =FALSE))
      }
      
       if (class(res)[1] == "NULL") {
          cat(paste0("Few or null data on numbers by age in landing and discard from Catch table for ", SPs[s], " in ", GSAs[g], "_\n\n"))
...
if (exists("Land")) {
  if (is.data.frame(Land)) {
    if (nrow(Land) > 0) {
      ...
      if (!user_SP)  SPs  <- sort(unique(Land$species))
      if (!user_MS)  MS   <- unique(Land$country)
      if (!user_GSA) GSAs <- sort(unique(Land$area))

      Land <- Land[Land$country %in% MS, ]
      Land <- Land[Land$area    %in% GSAs, ]
      Land <- Land[Land$species %in% SPs, ]

      if (nrow(Land) == 0) {
        stop("Land has no data after filtering (0 rows). Check MS/GSAs/SPs selections.")
      }
    ...
    cat("\n Land exists but is not a data.frame \n")
  }
} else {
  cat("\n No land data available \n")
}
...
```{r landing_coverage, results = 'asis'}
if (check_Land) {
    ...
    for (s in 1:length(SPs)) {
      ...
      if (!is.null(res)) {
        ...
        if (!RDBFIS_embedded_version_analysis) {
          ...
          tab1 <- aggregate(
            tab1$landings,
            by = list(tab1$country, tab1$year, tab1$area, tab1$gear, tab1$species),
            FUN = "sum"
          )
        ...
        # sempre stampato (come in Catch per res[[3]]/res[[4]])
        print(res[[2]])
        cat("\n\n\n")
...
```{r comp_Q_VL_fishery, results = 'asis'}
if (check_Land) {
...
```{r comp_YQ_fishery, results = 'asis'}
if (check_Land) {
...
```{r KS_test_land, results = 'asis'}

if (check_Land) {
  
  if(!RDBFIS_embedded_version_analysis){
  cat("\n## Kolmogorov-Smirnov test of landings LFDs\n")
  cat("\nThe Kolmogorov-Smirnov test provides cumulative length distribution plots by fishery and year. The test is performed on couples of years to assess if their distributions belong to the same population. \n")
...
```{r length_ind_land, results = 'asis'}

if (check_Land) {
  cat("\n## Main length size indicators\n")
  cat("\nThe time series of the main length size indicators by fishery was estimated and hereafter reported:\n")

  for (g in seq_along(GSAs)) {
    cat(paste0("\n### ", GSAs[g], "\n"))

    for (s in seq_along(SPs)) {
      cat(paste0("\n#### ", SPs[s], "\n"))

      # embedded switch (keep your original logic)
      if (RDBFIS_embedded_version_analysis) {
        res <- suppressMessages(
          MEDBS_length_ind(
            data = Land, type = "l",
            MS = MS, GSA = GSAs[g], SP = SPs[s],
            splines = c(0.2),
      ...
      if (is.list(res) && length(res) > 0) {
        ...
        # ---- summary table (optional but usually desired)
        if (!RDBFIS_embedded_version_analysis){
        if ("summary table" %in% names(res)) {
          ...
          cat("\n- Summary table not available in output.\n")
        }
        }
        # ---- combined plot (NEW output name)
        plot_name <- grep("^LengthIndicators_L", names(res), value = TRUE)
        if (length(plot_name) > 0) {
          cat(paste0("\nPlot of Main length size indicators for ", SPs[s], " - ", MS, " - ", GSAs[g], "\n"))
          suppressMessages(print(res[[plot_name[1]]]))
          cat("\n\n\n")
        } else {
          cat(paste0("\n- No combined plot produced for: ", SPs[s], " - ", MS, " - ", GSAs[g], ".\n"))
        }
...
```{r lengthclass_0_land, results = 'asis'}
if (check_Land) {
...
```{r yr_missing_length_land, results = 'asis'}
if (check_Land) {
...
```{r weight_minus1_land, results = 'asis'}
if (check_Land) {
...
```{r land_mean_weight_land, results = 'asis'}
if (check_Land) {
...
```{r plot_landing_ts, results = 'asis'}
if (check_Land) {
            ...
            if (class(res) == "list" & length(res) > 0) {
              ...
              for (t in 1:length(tables)) {
                ...
                print(tables[[t]] %>% kable("html") %>% kable_styling(font_size = 12) %>% kable_classic(full_width = TRUE))
				    ...
				    } # RDBFIS_embedded_version_analysis
				  } else {
...
if (exists("Disc")) {
  if (is.data.frame(Disc)) {
    if (nrow(Disc) > 0) {
      ...
      if (!user_SP)  SPs  <- sort(unique(Disc$species))
      if (!user_MS)  MS   <- unique(Disc$country)
      if (!user_GSA) GSAs <- sort(unique(Disc$area))

      Disc <- Disc[Disc$country %in% MS, ]
      Disc <- Disc[Disc$area    %in% GSAs, ]
      Disc <- Disc[Disc$species %in% SPs, ]

      if (nrow(Disc) == 0) {
        stop("Disc has no data after filtering (0 rows). Check MS/GSAs/SPs selections.")
      }
    ...
    cat("\n Disc exists but is not a data.frame \n")
  }
} else {
  cat("\n No Discard data available \n")
}
...
```{r discard_coverage, results = 'asis'}
if (check_Disc) {
...
```{r comp_disc_YQ_fishery, results = 'asis'}
if (check_Disc) {
...
```{r disc_mean_weight, results = 'asis'}
if (check_Disc) {
  cat("\n## Mean weight by year, gear and fishery aggregation\n")
            ...
            suppressMessages(
              suppressWarnings(
                tryCatch({
                  ...
                  on.exit({
                    ## Close any devices opened in this worker (including the temporary pdf)
                    repeat {
                      .dev_after <- grDevices::dev.list()
                      .dev_after_ids <- if (is.null(.dev_after)) integer(0) else as.integer(.dev_after)

                      .extra <- setdiff(.dev_after_ids, .dev_before_ids)
                      if (length(.extra) == 0) break

                      for (d in rev(.extra)) {
                        try(grDevices::dev.set(which = d), silent = TRUE)
                  ...
                  MEDBS_ks(data = Disc, type = "d", SP = SPs[s], MS = MS, GSA = GSAs[g], Rt = Rt)
                }, error = function(e) NULL)
              )
            )
          },
      ...
      for (s in 1:length(SPs)) {
        ...
        if (is.list(res) && length(res) > 0) {
          if (!is.null(res[[4]])) {
            ...
            cat(paste0("\n- No Kolmogorov-Smirnov test plot is available for the selected combination: ", SPs[s], " - ", MS, " - ", GSAs[g], ".\n"))
          }
        } else {
          cat(paste0("\n- No Kolmogorov-Smirnov test results are available for the selected combination: ", SPs[s], " - ", MS, " - ", GSAs[g], ".\n"))
        }
...
```{r length_ind_disc, results = 'asis'}
if (check_Disc) {
  cat("\n## Main length size indicators\n")
  cat("\nThe time series of the main length size indicators by fishery was estimated and hereafter reported:\n")
  ...
  for (g in seq_along(GSAs)) {
    ...
    for (s in seq_along(SPs)) {
      ...
      # embedded switch (same logic as landings chunk)
      if (RDBFIS_embedded_version_analysis) {
        res <- suppressMessages(
          MEDBS_length_ind(
            data = Disc, type = "d",
      ...
      if (is.list(res) && length(res) > 0) {
        ...
        # ---- combined plot
        plot_name <- grep("^LengthIndicators_D", names(res), value = TRUE)
        if (length(plot_name) > 0) {
          cat(paste0("\nPlot of Main length size indicators for ", SPs[s], " - ", MS, " - ", GSAs[g], "\n"))
          suppressMessages(print(res[[plot_name[1]]]))
          cat("\n\n\n")
        ...
        cat(paste0("\n- No main length size indicators estimated for the selected combination: ",
                   SPs[s], " - ", MS, " - ", GSAs[g], ".\n"))
      }
    }
  }
...
```{r lengthclass_0_disc, results = 'asis'}
if (check_Disc) {
  cat("\n## Check the number of records for which no demographic structure has been reported\n")
...
```{r yr_missing_length_disc, results = 'asis'}
if (check_Disc) {
  cat("\n## Check the presence of years with missing length distribution\n")
...
```{r weight_minus1_disc, results = 'asis'}
if (check_Disc) {
...
```{r plot_discard_ts, results = 'asis'}
if (check_Disc) {
...
if (exists("GP")) {
  if (is.data.frame(GP)) {
    if (nrow(GP) > 0) {
      ...
      if (!user_SP)  SPs  <- sort(unique(GP$species))
      if (!user_MS)  MS   <- unique(GP$country)
      if (!user_GSA) GSAs <- sort(unique(GP$area))

      GP <- GP[GP$country %in% MS, ]
      GP <- GP[GP$area    %in% GSAs, ]
      GP <- GP[GP$species %in% SPs, ]

      if (nrow(GP) == 0) {
        stop("GP has no data after filtering (0 rows). Check MS/GSAs/SPs selections.")
      }
    ...
    cat("\n GP exists but is not a data.frame \n")
  }
} else {
  cat("\n No growth parameters (GP) data available \n")
}
...
if (check_GP) {
  ...
  for (g in 1:length(GSAs)) {
    ...
    for (s in 1:length(SPs)) {
      cat(paste0("\n#### ", SPs[s], "\n"))
      res <- suppressMessages(MEDBS_GP_check(GP, SP = SPs[s], MS = MS, GSA = GSAs[g],all_plots=FALSE))
      ...
      if (class(res) == "list" & length(res) > 0) {
        ...
        index <- which(short_list == "VBGF_cum" | short_list == "VBGF_param")
  ...
  for (g in 1:length(GSAs)) {
    ...
    for (s in 1:length(SPs)) {
      ...
      if (class(res) == "list" & length(res) > 0) {
        ...
        index <- which((short_list == "LW_cum" | short_list == "LW_param") & !is.na(short_list2))
...
if (exists("MA")) {
  if (is.data.frame(MA)) {
    if (nrow(MA) > 0) {
      ...
      if (!user_SP)  SPs  <- sort(unique(MA$species))
      if (!user_MS)  MS   <- unique(MA$country)
      if (!user_GSA) GSAs <- sort(unique(MA$area))

      MA <- MA[MA$country %in% MS, ]
      MA <- MA[MA$area    %in% GSAs, ]
      MA <- MA[MA$species %in% SPs, ]

      if (nrow(MA) == 0) {
        stop("MA has no data after filtering (0 rows). Check MS/GSAs/SPs selections.")
      }
    ...
    cat("\n MA exists but is not a data.frame \n")
  }
} else {
  cat("\n No Maturity at Age (MA) data available \n")
}
...
if (exists("ML")) {
  if (is.data.frame(ML)) {
    if (nrow(ML) > 0) {
      ...
      if (!user_SP)  SPs  <- sort(unique(ML$species))
      if (!user_MS)  MS   <- unique(ML$country)
      if (!user_GSA) GSAs <- sort(unique(ML$area))

      ML <- ML[ML$country %in% MS, ]
      ML <- ML[ML$area    %in% GSAs, ]
      ML <- ML[ML$species %in% SPs, ]

      if (nrow(ML) == 0) {
        stop("ML has no data after filtering (0 rows). Check MS/GSAs/SPs selections.")
      }
    ...
    cat("\n ML exists but is not a data.frame \n")
  }
} else {
  cat("\n No Maturity at Length (ML) data available \n")
}
...
if (exists("SA")) {
  if (is.data.frame(SA)) {
    if (nrow(SA) > 0) {
      ...
      if (!user_SP)  SPs  <- sort(unique(SA$species))
      if (!user_MS)  MS   <- unique(SA$country)
      if (!user_GSA) GSAs <- sort(unique(SA$area))

      SA <- SA[SA$country %in% MS, ]
      SA <- SA[SA$area    %in% GSAs, ]
      SA <- SA[SA$species %in% SPs, ]

      if (nrow(SA) == 0) {
        stop("SA (SRA) has no data after filtering (0 rows). Check MS/GSAs/SPs selections.")
      }
    ...
    cat("\n SA exists but is not a data.frame \n")
  }
} else {
  cat("\n No Sex Ratio at age (SRA) data available \n")
}
...
if (exists("SL")) {
  if (is.data.frame(SL)) {
    if (nrow(SL) > 0) {
      ...
      if (!user_SP)  SPs  <- sort(unique(SL$species))
      if (!user_MS)  MS   <- unique(SL$country)
      if (!user_GSA) GSAs <- sort(unique(SL$area))

      SL <- SL[SL$country %in% MS, ]
      SL <- SL[SL$area    %in% GSAs, ]
      SL <- SL[SL$species %in% SPs, ]

      if (nrow(SL) == 0) {
        stop("SL (SRL) has no data after filtering (0 rows). Check MS/GSAs/SPs selections.")
      }
    ...
    cat("\n SL exists but is not a data.frame \n")
  }
} else {
  cat("\n No Sex Ratio at length (SRL) data available \n")
}
...
if (exists("ALK")) {
  if (is.data.frame(ALK)) {
    if (nrow(ALK) > 0) {
      ...
      if (!user_SP)  SPs  <- sort(unique(ALK$species))
      if (!user_MS)  MS   <- unique(ALK$country)
      if (!user_GSA) GSAs <- sort(unique(ALK$area))

      ALK <- ALK[ALK$country %in% MS, ]
      ALK <- ALK[ALK$area    %in% GSAs, ]
      ALK <- ALK[ALK$species %in% SPs, ]

      if (nrow(ALK) == 0) {
        stop("ALK has no data after filtering (0 rows). Check MS/GSAs/SPs selections.")
      }
    ...
    cat("\n ALK exists but is not a data.frame \n")
  }
} else {
  cat("\n No Age-Length Keys (ALK) data available \n")
}
...
if (check_ALK) {
  ...
  for (g in 1:length(GSAs)) {
    ...
    for (s in 1:length(SPs)) {
      ...
      if (class(res) == "list" & class(res)[1]!= "NULL") {
        l <- 1
  ...
  for (g in 1:length(GSAs)) {
    ...
    for (s in 1:length(SPs)) {
      ...
      res <- suppressMessages(MEDBS_ALK_MLAA(ALK, SP = SPs[s], MS = MS, GSA = GSAs[g]))
       ...
       if (nrow(ALK)>0 & class(res)[1]!= "NULL") {
          print(res)
          cat("\n\n\n")
              } else if (class(res)[1]== "NULL"){
        cat(paste0("\n- No ALK data are available for the selected combination: ", SPs[s], " - ", MS, " - ", GSAs[g], ".\n"))
