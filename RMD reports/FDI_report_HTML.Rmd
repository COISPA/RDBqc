...
tableA <- read.csv("D:\\Documents and Settings\\Utente\\Documenti\\GitHub\\RDBqc\\APPOGGIO\\Ioannis\\exported_dataFDI_CATCH.csv", sep=",") # fdi_a_catch
# tableG <- fdi_g_effort
tableH <- read.csv("D:\\Documents and Settings\\Utente\\Documenti\\GitHub\\RDBqc\\APPOGGIO\\Ioannis\\exported_dataSPATIAL_LAND.csv", sep=",") # fdi_h_spatial_landings
tableI <- read.csv("D:\\Documents and Settings\\Utente\\Documenti\\GitHub\\RDBqc\\APPOGGIO\\Ioannis\\exported_dataSPATIAL_FE.csv", sep=",")# fdi_i_spatial_effort
...
if (exists("tableG")) {
  if (class(tableG)=="data.frame") {
    if(nrow(tableG)>0){
      ...
      # if(!user_vessel_len){
      #   vessel_len <- unique(tableG$vessel_length)
      # }
      if(!user_fishtech){
...
if (check_tableG) {
  ...
  res <- suppressMessages(check_RD_FDI_G(data = tableG, verbose = FALSE))
    ...
    if (length(res) > 0) {
      ...
      df <- tableG[res,which(colnames(tableG)%in% c("country",                                                    "year","quarter","vessel_length","fishing_tech","gear_type",
            "target_assemblage","mesh_size_range","metier",
...
if (check_tableH) {
  ...
  res <- suppressMessages(check_RD_FDI_H(data = tableH, verbose = FALSE))
    ...
    if (length(res) > 0) {
      ...
      df <- tableH[res,which(colnames(tableH)%in% c("country",                                                    "year","quarter","vessel_length","fishing_tech","gear_type",
            "target_assemblage","mesh_size_range","metier",
...
if (check_tableI) {
  ...
  res <- suppressMessages(check_RD_FDI_I(data = tableI, verbose = FALSE))
    ...
    if (length(res) > 0) {
      ...
      df <- tableI[res,which(colnames(tableI)%in% c("country",                                                    "year","quarter","vessel_length","fishing_tech","gear_type",
            "target_assemblage","mesh_size_range","metier",
            "sub_region","rectangle_type","rectangle_lat","rectangle_lon",
            "c_square"))]
...
if (check_tableJ) {
  ...
  res <- suppressMessages(check_RD_FDI_J(data = tableJ, verbose = FALSE))
    ...
    if (length(res) > 0) {
      ...
      df <- tableJ[res,which(colnames(tableJ)%in% c("country",                                                    "year","vessel_length","fishing_tech","supra_region",
...
if (check_tableJ) {
  ...
  cat("\nThe FDI catch table was analyzed to return plots for four variables (total trips; total kW; total GT; total vessels) grouped by year, vessels length, and fishing techniques.\n")
...
if (check_tableA & check_tableG) {
  ...
  cat("\nBoth FDI table A (Catch) and G (Effort) were analysed to report the mutual consistency of both landings and effort data in table A and G.'\n")
...
if (check_tableA & check_tableH) {
  ...
  cat("\nBoth FDI table A (Catch) and H (Landing by rectangle) were analysed to report the mutual consistency of both landings data in table A and H.'\n")
...
#   cat("\nBoth FDI table G (Effort) and I (Effort by rectangle) were analysed to report the mutual consistency of both effort and spatial effort data in table G and I.'\n")
# 
#   res <- suppressMessages(FDI_cross_checks_AH(data1 = tableA, data2 = tableH, verbose = FALSE))
# 
#   if (!is.null(res)) {
