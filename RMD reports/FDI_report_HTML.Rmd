...
if (exists("tableH")) {
  if (class(tableH)=="data.frame") {
    if(nrow(tableH)>0){
      ...
      if(!user_SP){
        SPs <- unique(tableH$species)
      }
      if(!user_MS){
        MS  <- unique(tableH$country)
      }
      if(!user_GSA){
        GSAs <- unique(tableH$sub_region)
      }
...
if (exists("tableJ")) {
  if (class(tableJ)=="data.frame") {
    if(nrow(tableJ)>0){
      ...
      if(!user_GSA){
        GSAs <- unique(tableJ$principal_sub_region)
      }
      if(!user_vessel_len){
