...
task_ii2 <- read.csv("D:/OneDrive - Coispa Tecnologia & Ricerca S.C.A.R.L/RDB/data/GFCM-DCRF-Tasks_2018-2019/GFCM-DCRF-Tasks_2018-2019/task_ii2.csv", sep=";") #
task_ii2 <- task_ii2[task_ii2$GSA =="18",]
# task_ii2$GSA <- "99"

task_iii <- read.csv("D:/OneDrive - Coispa Tecnologia & Ricerca S.C.A.R.L/RDB/data/GFCM-DCRF-Tasks_2018-2019/GFCM-DCRF-Tasks_2018-2019/task_iii.csv", sep=";")
task_iii <- task_iii[task_iii$GSA =="18",]
# task_iii$GSA <- "99"

task_vii2 <- read.csv("D:/OneDrive - Coispa Tecnologia & Ricerca S.C.A.R.L/RDB/data/GFCM-DCRF-Tasks_2018-2019/GFCM-DCRF-Tasks_2018-2019/task_vii2.csv", sep=";") 
task_vii2 <- task_vii2[task_vii2$GSA =="18",]
# task_vii2$GSA <- "99"

task_vii31 <- read.csv("D:/OneDrive - Coispa Tecnologia & Ricerca S.C.A.R.L/RDB/data/GFCM-DCRF-Tasks_2018-2019/GFCM-DCRF-Tasks_2018-2019/task_vii31.csv", sep=";") 
task_vii31 <- task_vii31[task_vii31$GSA =="18",]
# task_vii31$GSA <- "99"

task_vii32 <- read.csv("D:/OneDrive - Coispa Tecnologia & Ricerca S.C.A.R.L/RDB/data/GFCM-DCRF-Tasks_2018-2019/GFCM-DCRF-Tasks_2018-2019/task_vii32.csv", sep=";") 
task_vii32 <- task_vii32[task_vii32$GSA =="18",]
# task_vii32$GSA <- "99"
...
if (exists("task_ii2")) {
  if (class(task_ii2)=="data.frame") {
    if(nrow(task_ii2)>0){
      ...
      if(!user_SP){
        SPs <- sort(unique(task_ii2$Species))
      }
      if(!user_MS){
        MS  <- unique(task_ii2$CPC)
      }
      if(!user_GSA){
        GSAs <- sort(unique(task_ii2$GSA))
      }
      if(!user_segments){
        Segments <- sort(unique(task_ii2$Segment))
      }
     
      task_ii2 <- task_ii2[task_ii2$Species %in% SPs & 
                       task_ii2$CPC == MS & 
                       task_ii2$GSA %in% GSAs, ]
...
if (check_task_ii2) {
  ...
  res <- check_RD_taskII2(data=task_ii2,verbose=FALSE)
  
  if (!is.null(res) & length(res)>0) {
    duplicates <- task_ii2[res,]
    if (nrow(duplicates)>0) {
      cat("\n", paste0("- _WARNING_ - ", nrow(duplicates), " replicated record/records in the data"), "\n")
      print(task_ii2[res,] %>% kable("html") %>% kable_styling(font_size = 12) %>% kable_classic(full_width=TRUE) )
      task_ii2 <- task_ii2[-res,]
    } else {
...
if (check_task_ii2) {
  ...
  res <- suppressMessages(check_EF_taskII2(data = task_ii2, verbose = FALSE))
...
for (g in 1:length(GSAs)) {
      ...
      for (s in 1:length(SPs)) {
        cat(paste0("\n#### ", SPs[s], "\n"))
         ...
         res <- suppressMessages(GFCM_cov_II2(data = task_ii2, MS = MS, SP = SPs[s], segment = Segments, GSA = GSAs[g], verbose = FALSE))
...
if (exists("task_iii")) {
  if (class(task_iii)=="data.frame") {
    if(nrow(task_iii)>0){
      ...
      if(!user_SP){
        SPs <- sort(unique(task_iii$Species))
      }
      if(!user_MS){
        MS  <- unique(task_iii$CPC)
      }
      if(!user_GSA){
        GSAs <- sort(unique(task_iii$GSA))
      }
      if(!user_segments){
        Segments <- sort(unique(task_iii$Segment))
      }
     
      task_iii <- task_iii[task_iii$Species %in% SPs & 
                       task_iii$CPC == MS & 
                       task_iii$GSA %in% GSAs, ]
...
if (check_task_iii) {
  ...
  res <- check_RD_taskIII(data=task_iii,verbose=FALSE)
  
  if (!is.null(res) & length(res)>0) {
    duplicates <- task_iii[res,]
    if (nrow(duplicates)>0) {
      cat("\n", paste0("- _WARNING_ - ", nrow(duplicates), " replicated record/records in the data"), "\n")
      print(task_iii[res,] %>% kable("html") %>% kable_styling(font_size = 12) %>% kable_classic(full_width=TRUE) )
      task_iii <- task_iii[-res,]
    } else {
...
if (check_task_iii) {
  ...
  res <- suppressMessages(check_EF_taskIII(data = task_iii, verbose = FALSE))
...
for (g in 1:length(GSAs)) {
      ...
      for (s in 1:length(SPs)) {
        cat(paste0("\n#### ", SPs[s], "\n"))
         ...
         res <- suppressMessages(GFCM_cov_task_iii(data = task_iii, MS = MS, SP = SPs[s], GSA = GSAs[g], verbose = FALSE))
...
if (exists("task_vii2")) {
  if (class(task_vii2)=="data.frame") {
    if(nrow(task_vii2)>0){
      ...
      if(!user_SP){
        SPs <- sort(unique(task_vii2$Species))
      }
      if(!user_MS){
        MS  <- unique(task_vii2$CPC)
      }
      if(!user_GSA){
        GSAs <- sort(unique(task_vii2$GSA))
      }
      
      task_vii2 <- task_vii2[task_vii2$Species %in% SPs & 
                       task_vii2$CPC == MS & 
                       task_vii2$GSA %in% GSAs, ]
...
if (check_task_vii2) {
  ...
  res <- check_RD_taskVII2(data=task_vii2,verbose=FALSE)
  
  if (!is.null(res)) {
    
    if (length(res)>0){
    duplicates <- task_vii2[res,]
    if (nrow(duplicates)>0) {
      cat("\n", paste0("- _WARNING_ - ", nrow(duplicates), " replicated record/records in the data"), "\n")
      print(task_vii2[res,] %>% kable("html") %>% kable_styling(font_size = 12) %>% kable_classic(full_width=TRUE) )
      task_vii2 <- task_vii2[-res,]
    } else {
...
if (check_task_vii2) {
  ...
  res <- suppressMessages(check_EF_taskVII2(data = task_vii2, verbose = FALSE))
...
for (g in 1:length(GSAs)) {
      ...
      for (s in 1:length(SPs)) {
        cat(paste0("\n#### ", SPs[s], "\n"))
         ...
         res <- suppressMessages(check_lw_TaskVII.2(data = task_vii2, MS = MS, SP = SPs[s], GSA = GSAs[g], verbose = FALSE))
...
for (g in 1:length(GSAs)) {
      ...
      for (s in 1:length(SPs)) {
        cat(paste0("\n#### ", SPs[s], "\n"))
         ...
         res <- suppressMessages(check_ldf_TaskVII.2(data = task_vii2, MS = MS, SP = SPs[s], GSA = GSAs[g], verbose = FALSE))
...
if (exists("task_vii31")) {
  if (class(task_vii31)=="data.frame") {
    if(nrow(task_vii31)>0){
      check_task_vii31 <- TRUE
      cat(paste0("\n# GFCM Size at first maturity (Table VII.3.1)\n"))
      
      
      if(!user_SP){
        SPs <- sort(unique(task_vii31$Species))
      }
      if(!user_MS){
        MS  <- unique(task_vii31$CPC)
      }
      if(!user_GSA){
        GSAs <- sort(unique(task_vii31$GSA))
      }
      
      task_vii31 <- task_vii31[task_vii31$Species %in% SPs & 
                       task_vii31$CPC == MS & 
                       task_vii31$GSA %in% GSAs, ]
...
if (check_task_vii31) {
  ...
  res <- check_RD_taskVII31(data=task_vii31,verbose=FALSE)
  
  if (!is.null(res)) {
    if (length(res)>0){
    duplicates <- task_vii31[res,]
    if (nrow(duplicates)>0) {
      cat("\n", paste0("- _WARNING_ - ", nrow(duplicates), " replicated record/records in the data"), "\n")
      print(task_vii31[res,] %>% kable("html") %>% kable_styling(font_size = 12) %>% kable_classic(full_width=TRUE) )
      task_vii31 <- task_vii31[-res,]
    } else {
...
if (check_task_vii31) {
  ...
  res <- suppressMessages(check_EF_TaskVII31(data = task_vii31, verbose = FALSE))
...
for (g in 1:length(GSAs)) {
      ...
      for (s in 1:length(SPs)) {
        cat(paste0("\n#### ", SPs[s], "\n"))
         ...
         res <- suppressMessages(check_l50_TaskVII.3.1(data = task_vii31, MS = MS, SP = SPs[s], GSA = GSAs[g], verbose = FALSE))
...
if (exists("task_vii32")) {
  if (class(task_vii32)=="data.frame") {
    if(nrow(task_vii32)>0){
      check_task_vii32 <- TRUE
      cat(paste0("\n# GFCM Biological information (Table VII.3.2, Maturity data)\n"))
      
      
      if(!user_SP){
        SPs <- sort(unique(task_vii32$Species))
      }
      if(!user_MS){
        MS  <- unique(task_vii32$CPC)
      }
      if(!user_GSA){
        GSAs <- sort(unique(task_vii32$GSA))
      }
      
      task_vii32 <- task_vii32[task_vii32$Species %in% SPs & 
                       task_vii32$CPC == MS & 
                       task_vii32$GSA %in% GSAs, ]
...
if (check_task_vii32) {
  ...
  res <- check_RD_TaskVII32(data=task_vii32,verbose=FALSE)
  
  if (!is.null(res)) {
    if (length(res)>0){
    duplicates <- task_vii32[res,]
    if (nrow(duplicates)>0) {
      cat("\n", paste0("- _WARNING_ - ", nrow(duplicates), " replicated record/records in the data"), "\n")
      print(task_vii32[res,] %>% kable("html") %>% kable_styling(font_size = 12) %>% kable_classic(full_width=TRUE) )
      task_vii32 <- task_vii32[-res,]
    } else {
...
if (check_task_vii32) {
  ...
  res <- suppressMessages(check_EF_TaskVII32(data = task_vii32, verbose = FALSE))
...
for (g in 1:length(GSAs)) {
      ...
      for (s in 1:length(SPs)) {
        cat(paste0("\n#### ", SPs[s], "\n"))
         ...
         res <- suppressMessages(check_species_catfau_TaskVII.3.2(data = task_vii32,species=catfau_check, matsex=sex_mat, MS = MS, SP = SPs[s], GSA = GSAs[g], verbose = FALSE))
...
for (g in 1:length(GSAs)) {
      ...
      for (s in 1:length(SPs)) {
        cat(paste0("\n#### ", SPs[s], "\n"))
         ...
         res <- suppressMessages(check_lw_TaskVII.3.2(data = task_vii32, MS = MS, SP = SPs[s], GSA = GSAs[g], verbose = FALSE))
