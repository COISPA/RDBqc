...
Catch <- read.table("D:/OneDrive - Coispa Tecnologia & Ricerca S.C.A.R.L/RDB3/test RDBqc/commercial/catch.csv", sep = ",", header = TRUE)
# Land <- read.table("D:/OneDrive - Coispa Tecnologia & Ricerca S.C.A.R.L/RDB3/test RDBqc/commercial/landings.csv", sep = ",", header = TRUE)
# Disc <- read.table("D:/OneDrive - Coispa Tecnologia & Ricerca S.C.A.R.L/RDB3/test RDBqc/commercial/discards.csv", sep = ",", header = TRUE)
# ML <- read.table("D:/OneDrive - Coispa Tecnologia & Ricerca S.C.A.R.L/RDB3/test RDBqc/commercial/ml.csv", sep = ",", header = TRUE)
# MA <- read.table("D:/OneDrive - Coispa Tecnologia & Ricerca S.C.A.R.L/RDB3/test RDBqc/commercial/ma.csv", sep = ",", header = TRUE)
# SL <- read.table("D:/OneDrive - Coispa Tecnologia & Ricerca S.C.A.R.L/RDB3/test RDBqc/commercial/srl.csv", sep = ",", header = TRUE)
# SA <- read.table("D:/OneDrive - Coispa Tecnologia & Ricerca S.C.A.R.L/RDB3/test RDBqc/commercial/sra.csv", sep = ",", header = TRUE)
# GP <- read.table("D:/OneDrive - Coispa Tecnologia & Ricerca S.C.A.R.L/RDB3/test RDBqc/commercial/gp.csv", sep = ",", header = TRUE)
# ALK <- read.table("D:/OneDrive - Coispa Tecnologia & Ricerca S.C.A.R.L/RDB3/test RDBqc/commercial/alk.csv", sep = ",", header = TRUE)

SPs <- sort(c("HKE","DPS","NEP","ANE","MUT"))
MS <- "ITA"
GSAs <- "GSA 18"
end_year <- 2022
```
...
# `{r catch_duplicated_records, results = 'asis'} # if (check_Catch) { #   cat("\n## No replicated records in Catch data\n") #   cat("\nData included in the catch table were checked for the presence of duplicated records. The result of the check is here reported:\n") #  #   duplicates <- Catch[0, ] #  #   g <- 1 #   for (g in 1:length(GSAs)) { #     s <- 1 #     for (s in 1:length(SPs)) { #       res <- MEDBS_check_duplicates(data = Catch, type = "c", SP = SPs[s], MS = MS, GSA = GSAs[g], verbose = FALSE) #       if (!is.null(res)) { #         duplicates <- rbind(duplicates, res) #       } #     } #   } #  #   if (nrow(duplicates) > 0) { #     cat("\n", paste0("- _WARNING_ - ", nrow(duplicates), " replicated record/records in the data"), "\n") #     duplic <- data.frame(id = duplicates[, 1]) #     print(duplic %>% kable("html") %>% kable_styling(font_size = 12) %>% kable_classic(full_width = TRUE)) #     Catch <- Catch[-which(rownames(Catch) %in% as.numeric(rownames(duplicates))), ] #   } else { #     cat("\n", paste0("- No replicated records in Catch data"), "\n") #   } #  #   cat("\n*Attention*: eventual duplicated records will be removed from catch data for the following analysis \n") # } # check_Catch #`

```{r catch_duplicated_records, results = 'asis', message=FALSE, warning=FALSE}
if (check_Catch) {

  # ------------------------------------------------------------
  # Section header (sequential rendering)
  # ------------------------------------------------------------
  cat("\n## No replicated records in Catch data\n")
  cat("\nData included in the catch table were checked for the presence of duplicated records. The result of the check is here reported:\n")

  # ------------------------------------------------------------
  # Cross-platform parallel backend (Windows, Linux, macOS)
  # - No library() calls here (assumes future/furrr are already available in the document)
  # - Do not print from workers, return only objects
  # ------------------------------------------------------------
  old_plan <- future::plan()
  on.exit(future::plan(old_plan), add = TRUE)

  workers_n <- max(1, future::availableCores() - 1)
  future::plan(future::multisession, workers = workers_n)

  # Deterministic RNG streams (safe even if functions become stochastic in future)
  RNGkind("L'Ecuyer-CMRG")
  set.seed(1)

  # ------------------------------------------------------------
  # Build deterministic task grid (GSA x SP)
  # ------------------------------------------------------------
  tasks <- tidyr::expand_grid(GSA = GSAs, SP = SPs) %>%
    dplyr::arrange(GSA, SP)

  # ------------------------------------------------------------
  # Worker function (no printing, return only objects)
  # ------------------------------------------------------------
  run_one_dup <- function(GSA, SP) {

    # Return a list; store duplicates as a list-element to avoid coercion issues
    out <- list(
      GSA = GSA,
      SP  = SP,
      dup = list(NULL)
    ...
    res <- tryCatch(
      MEDBS_check_duplicates(data = Catch, type = "c", SP = SP, MS = MS, GSA = GSA, verbose = FALSE),
      error = function(e) e
    )

    if (inherits(res, "error")) {
      # In case of error, return NULL duplicates for this combination (do not stop the report)
      out$dup <- list(NULL)
      return(out)
    }
  ...
  # ------------------------------------------------------------
  # Parallel computation (in-memory)
  # ------------------------------------------------------------
  res_list <- furrr::future_pmap(
    .l = list(tasks$GSA, tasks$SP),
    .f = run_one_dup,
    .options = furrr::furrr_options(seed = TRUE)
  )

  # ------------------------------------------------------------
  # Collect results deterministically (no bind_rows to avoid vctrs issues)
  # ------------------------------------------------------------
  duplicates <- Catch[0, ]
  ...
  for (i in seq_along(res_list)) {
    d <- res_list[[i]]$dup[[1]]
    if (!is.null(d)) {
      duplicates <- rbind(duplicates, d)
    }
  }

  # ------------------------------------------------------------
  # Report output (same behaviour and structure as original chunk)
  # ------------------------------------------------------------
...
# `{r catch_disaggregated_records, results = 'asis'} # if (check_Catch) { #   cat("\n## Presence of disaggregated records\n") #   cat("\nData included in the catch table were checked for the presence of disaggregated records. The result of the check is here reported:\n") #  #   disaggregated <- Catch[0, ] #  #   g <- 1 #   for (g in 1:length(GSAs)) { #     s <- 1 #     for (s in 1:length(SPs)) { #       res <- MEDBS_check_disaggregated(data = Catch, type = "c", SP = SPs[s], MS = MS, GSA = GSAs[g], verbose = FALSE) #       if (!is.null(res)) { #         disaggregated <- rbind(disaggregated, res) #       } #     } #   } #  #   if (nrow(disaggregated) > 0) { #     cat("\n", paste0("- _WARNING_ - ", nrow(disaggregated), " disaggregated record/records in the data"), "\n") #     print(disaggregated %>% kable("html") %>% kable_styling(font_size = 12) %>% kable_classic(full_width = TRUE)) #   } else { #     cat("\n", paste0("- No disaggregated records in Catch data"), "\n") #   } #  # } # check_Catch #`

```{r catch_disaggregated_records, results = 'asis', message=FALSE, warning=FALSE}
if (check_Catch) {
  ...
  # ------------------------------------------------------------
  # Cross-platform parallel backend (Windows, Linux, macOS)
  # - No library() calls here (assumes future/furrr/tidyr/dplyr are already available in the document)
  # - Do not print from workers, return only objects
  # ------------------------------------------------------------
  old_plan <- future::plan()
  on.exit(future::plan(old_plan), add = TRUE)
  ...
  # Worker function (no printing, return only objects)
  # ------------------------------------------------------------
  run_one_disagg <- function(GSA, SP) {
    ...
    res <- tryCatch(
      MEDBS_check_disaggregated(data = Catch, type = "c", SP = SP, MS = MS, GSA = GSA, verbose = FALSE),
      error = function(e) e
    )
  ...
  # ------------------------------------------------------------
  # Collect results deterministically (avoid bind_rows to prevent vctrs coercion issues)
  # ------------------------------------------------------------
  ...
  for (i in seq_along(res_list)) {
    d <- res_list[[i]]$dis[[1]]
    if (!is.null(d)) {
      disaggregated <- rbind(disaggregated, d)
    }
  }
...
if (check_Catch) {
  ...
  # ------------------------------------------------------------
  # Worker function (no printing, return only objects)
  # - res_status: "no_data" (NULL), "ok" (length(res)==0 or >0), "error"
  # - missing_years: list-column containing the vector of missing years (possibly length 0)
  # ------------------------------------------------------------
  run_one_missing <- function(GSA, SP) {

    out <- list(
      GSA = GSA,
      SP  = SP,
      status = "no_data",
      missing_years = list(NULL)
    )

    res <- tryCatch(
      MEDBS_check_missing_years(data = Catch, end_year, type = "c", SP = SP, MS = MS, GSA = GSA, verbose = FALSE),
      error = function(e) e
    )

    if (inherits(res, "error")) {
      out$status <- "error"
      out$missing_years <- list(NULL)
      return(out)
    }
    ...
    # Not NULL: either length 0 (no missing) or >0 (missing years)
    out$status <- "ok"
    out$missing_years <- list(res)
    out
  }
  ...
  # Deterministic O(1) lookup (avoid bind_rows)
  keys <- paste(tasks$GSA, tasks$SP, sep = "||")
  res_map <- setNames(res_list, keys)

  get_res <- function(g, sp) {
    res_map[[paste(g, sp, sep = "||")]]
  }

  # ------------------------------------------------------------
  # Sequential rendering with stable headings and numbering
  # (same title structure as original: no additional headers)
  # ------------------------------------------------------------
  for (g in 1:length(GSAs)) {
    for (s in 1:length(SPs)) {

      row <- get_res(GSAs[g], SPs[s])
      ...
      if (identical(row$status, "error")) {
        cat(paste0("\n- No Catch data for the selected combination: ", SPs[s], " - ", MS, " - ", GSAs[g], "\n"))
        next
      }

      if (identical(row$status, "no_data")) {
        cat(paste0("\n- No Catch data for the selected combination: ", SPs[s], " - ", MS, " - ", GSAs[g], "\n"))
        next
      }

      # status == "ok"
      miss <- row$missing_years[[1]]

      if (!is.null(miss) && length(miss) > 0) {
        cat(paste0(
          "\n- The following years are missing in the Catch data time series (",
          SPs[s], " - ", MS, " - ", GSAs[g], "): ",
          paste(miss, collapse = ", "),
          "\n"
        ))
        ...
        cat(paste0(
          "\n- No missing years in the Catch data time series for the selected combination: ",
          SPs[s], " - ", MS, " - ", GSAs[g], "\n"
        ))
      }
    } # for cycle SP
...
# `{r catch_coverage, results = 'asis',fig.height=6,fig.width=9} #  # if (check_Catch) { #   cat("\n## Catch data time series\n") #  #   for (g in 1:length(GSAs)) { #     cat(paste0("\n### ", GSAs[g], "\n")) #     s <- 1 #     for (s in 1:length(SPs)) { #       cat(paste0("\n#### ", SPs[s], "\n")) #       res <- suppressMessages(MEDBS_Catch_coverage(Catch, SP = SPs[s], MS = MS, GSA = GSAs[g])) #       tab1 <- res[[1]] #       tab2 <- res[[2]] #  #       if (!is.null(res)) { #         suppressMessages(tab1 <- data.frame(tab1 %>% group_by(country, year, area, species) %>% summarise(landings = sum(landings, na.rm = TRUE)))) #  #         if (nrow(tab1) > 0) { #           cat(paste0("\n_Coverage of landing data for ", SPs[s], " in ", GSAs[g], "_\n")) #  #           print(tab1 %>% kable("html") %>% kable_styling(font_size = 12) %>% kable_classic(full_width = TRUE)) #  #           cat("\n\n\n") #           print(res[[3]]) #           cat("\n\n\n") #         } else { #           cat("\n\n\n- No data for landings") #         } #  #         suppressMessages(tab2 <- data.frame(tab2 %>% group_by(country, year, area, species) %>% summarise(discards = sum(discards, na.rm = TRUE)))) #         if (nrow(tab2) > 0) { #           cat(paste0("\n_Coverage of discard data for ", SPs[s], " in ", GSAs[g], "_")) #           print(tab2 %>% kable("html") %>% kable_styling(font_size = 12) %>% kable_classic(full_width = TRUE)) #           cat("\n\n\n") #  #           print(res[[4]]) #           cat("\n\n\n") #         } else { #           cat("\n\n\n- No data for discards") #         } #       } else { #         cat(paste0("\n- No Catch data for the selected combination: ", SPs[s], " - ", MS, " - ", GSAs[g], "\n")) #       } #     } #   } # } # check_Catch #`

```{r catch_coverage, results = 'asis', fig.height=6, fig.width=9, message=FALSE, warning=FALSE, fig.show='hold'}
if (check_Catch) {
  ...
  # ------------------------------------------------------------
  # Worker function (no printing, return only objects)
  # - Store tables and plots as list-elements (in-memory)
  # - Keep interface/output unchanged at rendering stage
  # ------------------------------------------------------------
  run_one_cov <- function(GSA, SP) {
    ...
    res <- tryCatch(
      suppressMessages(MEDBS_Catch_coverage(Catch, SP = SP, MS = MS, GSA = GSA)),
      error = function(e) e
    )

    if (inherits(res, "error")) {
      out$msg <- paste0("MEDBS_Catch_coverage failed: ", conditionMessage(res))
      return(out)
    }

    if (is.null(res)) {
      out$msg <- paste0("No Catch data for the selected combination: ", SP, " - ", MS, " - ", GSA)
      return(out)
    }

    # Preserve original structure: res[[1]] land table, res[[2]] discard table, res[[3]] land plot, res[[4]] discard plot
    tab1 <- res[[1]]
    tab2 <- res[[2]]

    # Aggregate as in original chunk (in worker, no printing)
    tab1a <- tryCatch(
      suppressMessages(data.frame(tab1 %>% dplyr::group_by(country, year, area, species) %>% dplyr::summarise(landings = sum(landings, na.rm = TRUE), .groups = "drop"))),
      error = function(e) NULL
    )

    tab2a <- tryCatch(
      suppressMessages(data.frame(tab2 %>% dplyr::group_by(country, year, area, species) %>% dplyr::summarise(discards = sum(discards, na.rm = TRUE), .groups = "drop"))),
      error = function(e) NULL
    )

    out$ok <- TRUE
    out$tab_land  <- list(tab1a)
    out$tab_disc  <- list(tab2a)
    out$plot_land <- list(res[[3]])
    out$plot_disc <- list(res[[4]])
    out
  }
  ...
  # Deterministic O(1) lookup (avoid bind_rows to prevent vctrs coercion issues with grobs)
  keys <- paste(tasks$GSA, tasks$SP, sep = "||")
  res_map <- setNames(res_list, keys)
  ...
  # ------------------------------------------------------------
  # Robust plot renderer for knitr (ggplot, grob/gtable, recordedplot)
  # ------------------------------------------------------------
  ...
  # ------------------------------------------------------------
  # Sequential rendering with stable headings and numbering (same as original)
  # ------------------------------------------------------------
  for (g in 1:length(GSAs)) {
    cat(paste0("\n### ", GSAs[g], "\n"))
    ...
    for (s in 1:length(SPs)) {
      ...
      if (!isTRUE(row$ok)) {
        # Keep original message behaviour
        if (!is.na(row$msg)) {
          cat(paste0("\n- ", row$msg, "\n"))
        } else {
          cat(paste0("\n- No Catch data for the selected combination: ", SPs[s], " - ", MS, " - ", GSAs[g], "\n"))
      ...
      # Landing coverage table + plot
      tab1a <- row$tab_land[[1]]
      if (!is.null(tab1a) && nrow(tab1a) > 0) {
        cat(paste0("\n_Coverage of landing data for ", SPs[s], " in ", GSAs[g], "_\n"))
        print(tab1a %>% kable("html") %>% kable_styling(font_size = 12) %>% kable_classic(full_width = TRUE))
        cat("\n\n\n")
        render_plot(row$plot_land[[1]])
        cat("\n\n\n")
      } else {
        cat("\n\n\n- No data for landings")
      }

      # Discard coverage table + plot
      tab2a <- row$tab_disc[[1]]
      if (!is.null(tab2a) && nrow(tab2a) > 0) {
        cat(paste0("\n_Coverage of discard data for ", SPs[s], " in ", GSAs[g], "_"))
        print(tab2a %>% kable("html") %>% kable_styling(font_size = 12) %>% kable_classic(full_width = TRUE))
        cat("\n\n\n")
        render_plot(row$plot_disc[[1]])
        cat("\n\n\n")
      } else {
...
# `{r catch_SOP, results = 'asis',fig.height=6,fig.width=9} # if (check_Catch) { #   cat("\n## Check of Sum of Products (SOP)\n") #   cat(paste0("\nRecords for which the Sum Of Product (SOP) in weight differs in absolute terms more then ", SOP_threshold, "% threshold chosen from the landings or discards volumes declared.\n")) #  #   for (g in 1:length(GSAs)) { #     cat(paste0("\n### ", GSAs[g], "\n")) #     s <- 1 #     for (s in 1:length(SPs)) { #       cat(paste0("\n#### ", SPs[s], "\n")) #       res <- suppressMessages(MEDBS_SOP(data=Catch, SP = SPs[s], MS = MS, GSA = GSAs[g], threshold = SOP_threshold, verbose = FALSE)) #  #       if (nrow(res) > 0) { #         cat(paste0("\n_Critical values of SOP for ", SPs[s], " in ", GSAs[g], "_\n\n")) #         print(res %>% kable("html") %>% kable_styling(font_size = 12) %>% kable_classic(full_width = TRUE)) #       } else { #         cat(paste0("\n_No critical values of SOP detected for ", SPs[s], " in ", GSAs[g], "_\n\n")) #       } #     } #   } # } # check_Catch #`

```{r catch_SOP, results = 'asis', fig.height=6, fig.width=9, message=FALSE, warning=FALSE}
if (check_Catch) {
  ...
  cat("\n## Check of Sum of Products (SOP)\n")
  cat(paste0(
    "\nRecords for which the Sum Of Product (SOP) in weight differs in absolute terms more then ",
    SOP_threshold,
    "% threshold chosen from the landings or discards volumes declared.\n"
  ))
  ...
  run_one_sop <- function(GSA, SP) {
    ...
    out <- list(
      ...
      ok  = FALSE,
      msg = NA_character_,
      tab = list(NULL)   # store results as list-element (in-memory)
    )

    res <- tryCatch(
      suppressMessages(
        MEDBS_SOP(data = Catch, SP = SP, MS = MS, GSA = GSA, threshold = SOP_threshold, verbose = FALSE)
      ),
      error = function(e) e
    )

    if (inherits(res, "error")) {
      out$msg <- paste0("MEDBS_SOP failed: ", conditionMessage(res))
      return(out)
    }
...
# `{r catch_LW, results = 'asis',fig.height=6,fig.width=9} # if (check_Catch) { #   cat("\n## Check of length weight in landing and discard from Catch table\n") #    #  #   for (g in 1:length(GSAs)) { #     cat(paste0("\n### ", GSAs[g], "\n")) #     s <- 1 #     for (s in 1:length(SPs)) { #       cat(paste0("\n#### ", SPs[s], "\n")) #       res <- suppressMessages(MEDBS_Catch_LW(data=Catch, SP = SPs[s], MS = MS, GSA = GSAs[g], verbose = FALSE)) # print(res) #     } #   } # } # check_Catch #`

```{r catch_LW, results = 'asis', fig.height=6, fig.width=9, message=FALSE, warning=FALSE, fig.show='hold'}
if (check_Catch) {
  ...
  # ------------------------------------------------------------
  # Cross-platform parallel backend (Windows, Linux, macOS)
  # No library() calls here. It is assumed that future/furrr/tidyr/dplyr
  # (and the RDBqc package) are already available in the document/session.
  # ------------------------------------------------------------
  old_plan <- future::plan()
  on.exit(future::plan(old_plan), add = TRUE)
  ...
  # ------------------------------------------------------------
  # Worker function (no printing, return only objects)
  # MEDBS_Catch_LW returns a plot (ggplot/ggpubr object) or NULL if no data
  # ------------------------------------------------------------
  run_one_lw <- function(GSA, SP) {
    ...
    out <- list(
      ...
      ok   = FALSE,
      msg  = NA_character_,
      plot = list(NULL)  # keep as list-element (in-memory)
    )

    res <- tryCatch(
      suppressMessages(MEDBS_Catch_LW(data = Catch, SP = SP, MS = MS, GSA = GSA, verbose = FALSE)),
      error = function(e) e
    )

    if (inherits(res, "error")) {
      out$msg <- paste0("MEDBS_Catch_LW failed: ", conditionMessage(res))
      return(out)
    }
  ...
  # Deterministic O(1) lookup (avoid bind_rows to prevent coercion issues with grobs)
  keys <- paste(tasks$GSA, tasks$SP, sep = "||")
  res_map <- setNames(res_list, keys)
  ...
  # ------------------------------------------------------------
  # Sequential rendering with stable headings and numbering (same structure as original)
  # ------------------------------------------------------------
  for (g in 1:length(GSAs)) {
    cat(paste0("\n### ", GSAs[g], "\n"))
    ...
    for (s in 1:length(SPs)) {
      ...
      # Print the plot (landing + discard panels) returned by MEDBS_Catch_LW
      p <- row$plot[[1]]
      if (!is.null(p)) {
        cat("\n")
        render_plot(p)
...
# ```{r catch_CAA, results = 'asis',fig.height=6,fig.width=9}
# if (check_Catch) {
#   cat("\n## Check the numbers by age in landing and discard from Catch table\n")
#   
# 
#   for (g in 1:length(GSAs)) {
#     cat(paste0("\n### ", GSAs[g], "\n"))
...
#     for (s in 1:length(SPs)) {
#       cat(paste0("\n#### ", SPs[s], "\n"))
#       res <- suppressMessages(MEDBS_Catch_CAA(data=Catch, SP = SPs[s], MS = MS, GSA = GSAs[g], verbose = FALSE))
# print(res)
#     }
#   }
# } # check_Catch
...
```{r catch_CAA, results = 'asis', fig.height=6, fig.width=9, message=FALSE, warning=FALSE}
if (check_Catch) {
  ...
  workers_n <- max(1, future::availableCores() - 1)
  workers_n <- min(workers_n, nrow(tasks))
  future::plan(future::multisession, workers = workers_n)

  # ------------------------------------------------------------
  # Worker function (no printing, return only objects)
  # - status: "ok", "no_data", "error"
  # - plot: list-column containing ggplot (or NULL)
  # ------------------------------------------------------------
  run_one_caa <- function(GSA, SP) {
    ...
    res <- tryCatch(
      suppressMessages(
        MEDBS_Catch_CAA(data = Catch, SP = SP, MS = MS, GSA = GSA, verbose = FALSE)
      ),
      error = function(e) e
    )
    ...
    if (inherits(res, "error")) {
      out$status <- "error"
      out$plot <- list(NULL)
      out$err_msg <- conditionMessage(res)
      return(out)
    }
  ...
  # ------------------------------------------------------------
  # Sequential rendering with stable headings and ordering
  # (keeps the same structure of the original nested for-loops)
  # ------------------------------------------------------------
  for (g in 1:length(GSAs)) {
    cat(paste0("\n### ", GSAs[g], "\n"))
    ...
    for (s in 1:length(SPs)) {
      ...
      if (is.null(row)) {
        cat(paste0("\nNo data for the selected combination: ", SPs[s], " - ", MS, " - ", GSAs[g], "\n"))
        next
      }

      if (identical(row$status, "error")) {
        cat(paste0(
          "\nNo plot produced for the selected combination (error): ",
          SPs[s], " - ", MS, " - ", GSAs[g],
          if (!is.null(row$err_msg)) paste0("\nDetails: ", row$err_msg) else "",
          "\n"
        ))
        next
      }
...
```{r landings_duplicated_records, results = 'asis', message=FALSE, warning=FALSE}
if (check_Land) {
  cat("\n## No replicated records in Landing data \n")
  cat("\nData included in the landing table were checked for the presence of duplicated records. The result of the check is here reported:\n")
  ...
  # ------------------------------------------------------------
  # Worker function:
  # - same "res" semantics as original
  # - tryCatch only prevents stopping: on error returns NULL
  # - no visible output changes (only message() for optional logging)
  # ------------------------------------------------------------
  run_one_dup_land <- function(GSA, SP) {
    tryCatch(
      ...
      error = function(e) {
        message(paste0(
          "MEDBS_check_duplicates error (SP=", SP,
          ", MS=", MS, ", GSA=", GSA, "): ", conditionMessage(e)
        ))
        NULL
      }
    )
  ...
  # ------------------------------------------------------------
  # Parallel computation (deterministic order via arranged tasks)
  # ------------------------------------------------------------
  res_list <- furrr::future_pmap(
    .l = list(tasks$GSA, tasks$SP),
    .f = run_one_dup_land,
  ...
  # ------------------------------------------------------------
  # Sequential aggregation: identical logic to original nested loops
  # ------------------------------------------------------------
  for (i in seq_along(res_list)) {
    res <- res_list[[i]]
    if (!is.null(res)) {
  ...
  # ------------------------------------------------------------
  # Reporting + cleaning: identical logic and texts to original
  # ------------------------------------------------------------
  if (nrow(duplicates) > 0) {
    cat("\n", paste0("- _WARNING_ - ", nrow(duplicates), " replicated record/records in the data"), "\n")
    duplic <- data.frame(id = duplicates[, 1])
...
```{r landings_disaggregated_records, results = 'asis', message=FALSE, warning=FALSE}
if (check_Land) {
  cat("\n## Presence of disaggregated records\n")
  cat("\nData included in the landings table were checked for the presence of disaggregated records. The result of the check is here reported:\n")
  ...
  # - no visible output changes (only message() for optional logging)
  # ------------------------------------------------------------
  run_one_disagg_land <- function(GSA, SP) {
    tryCatch(
      ...
      error = function(e) {
        message(paste0(
          "MEDBS_check_disaggregated error (SP=", SP,
          ", MS=", MS, ", GSA=", GSA, "): ", conditionMessage(e)
        ))
        NULL
      }
  ...
  # ------------------------------------------------------------
  # Reporting: identical logic and texts to original
  # ------------------------------------------------------------
  if (nrow(disaggregated) > 0) {
    cat("\n", paste0("- _WARNING_ - ", nrow(disaggregated), " disaggregated record/records in the data"), "\n")
    print(disaggregated %>% kable("html") %>% kable_styling(font_size = 12) %>% kable_classic(full_width = TRUE))
...
```{r land_missing_years, results = 'asis', message=FALSE, warning=FALSE}
if (check_Land) {
  cat("\n## Check missing years in time series\n")
  cat("\nLanding data where checked for the continuity of the time series, identifying the presence of missing years:\n")
  ...
  # - no visible output changes (only message() for optional logging)
  # ------------------------------------------------------------
  run_one_missing_land <- function(GSA, SP) {
    tryCatch(
      ...
      error = function(e) {
        message(paste0(
          "MEDBS_check_missing_years error (SP=", SP,
          ", MS=", MS, ", GSA=", GSA, "): ", conditionMessage(e)
        ))
        NULL
      }
  ...
  res_list <- furrr::future_pmap(
    .l = list(tasks$GSA, tasks$SP),
    .f = run_one_missing_land,
    .options = furrr::furrr_options(seed = TRUE)
  )

  # ------------------------------------------------------------
  # Sequential rendering: identical logic and order to original loops
  # ------------------------------------------------------------
  g <- 1
  for (g in 1:length(GSAs)) {
    s <- 1
...
if (check_Land) {
  ...
  # - same "res" semantics as original
  # - tryCatch only prevents stopping: on error returns NULL
  # - no visible output added
  # ------------------------------------------------------------
  run_one_landing_cov <- function(g, s) {
    tryCatch(
      suppressMessages(
        MEDBS_Landing_coverage(Land, SP = SPs[s], MS = MS, GSA = GSAs[g])
      ),
      error = function(e) NULL
    )
  }
  ...
  # ------------------------------------------------------------
  # Parallel computation (order matches original nested loops:
  # g outer, s inner)
  # ------------------------------------------------------------
  res_list <- furrr::future_pmap(
    .l = list(tasks$g, tasks$s),
    .f = run_one_landing_cov,
    .options = furrr::furrr_options(seed = TRUE)
  )

  # ------------------------------------------------------------
  # Sequential rendering: identical logic, texts, and order
  # ------------------------------------------------------------
  g <- 1
  for (g in 1:length(GSAs)) {
    cat(paste0("\n### ", GSAs[g], "\n"))
...
if (check_Land) {
  ...
  # ------------------------------------------------------------
  # Worker function:
  # - preserves "res" usage expected by the original code (nrow(res))
  # - tryCatch only prevents stopping
  # - on error (or NULL) returns an empty data.frame, so that nrow(res) == 0
  #   and the original else-branch is triggered
  # ------------------------------------------------------------
  run_one_comp_q_vl <- function(g, s) {
    out <- tryCatch(
      suppressMessages(
        MEDBS_comp_land_Q_VL_fishery(data = Land, MS = MS, GSA = GSAs[g], SP = SPs[s])
      ),
      error = function(e) NULL
    )
    if (is.null(out)) {
  ...
  # ------------------------------------------------------------
  # Parallel computation (order matches original nested loops: g outer, s inner)
  # ------------------------------------------------------------
  res_list <- furrr::future_pmap(
    .l = list(tasks$g, tasks$s),
    .f = run_one_comp_q_vl,
  ...
  # ------------------------------------------------------------
  # Sequential rendering: identical logic, texts, and order to original loops
  # ------------------------------------------------------------
  for (g in 1:length(GSAs)) {
    cat(paste0("\n#### ", GSAs[g], "\n"))
    s <- 1
...
```{r comp_YQ_fishery, results = 'asis', fig.height=6, fig.width=9, message=FALSE, warning=FALSE}
if (check_Land) {
  cat("\n### Comparison between landings in weight reported by quarter and at year time frame by gear and fishery combination\n")
  ...
  #   and the original else-branch is triggered
  # ------------------------------------------------------------
  run_one_comp_yq <- function(g, s) {
    out <- tryCatch(
      suppressMessages(
        MEDBS_comp_land_YQ_fishery(data = Land, MS = MS, GSA = GSAs[g], SP = SPs[s])
      ),
      error = function(e) {
        message(paste0(
          "MEDBS_comp_land_YQ_fishery error (SP=", SPs[s],
          ", MS=", MS, ", GSA=", GSAs[g], "): ", conditionMessage(e)
        ))
        NULL
      }
    )
...
#         if (!is.null(res[[4]])) {
#           grid::grid.newpage()
#           invisible(grid::grid.draw(res[[4]]))
#         }
#         cat("\n\n\n")
# 
# 
...
if (check_Land) {
  ...
  .ks_res_mat <- local({
    ...
    .res_list <- tryCatch(
          ...
          suppressMessages(
            suppressWarnings(
              tryCatch({
                ...
                .dev_before_ids <- if (is.null(.dev_before)) integer(0) else as.integer(.dev_before)

                .tmp_pdf <- tempfile(fileext = ".pdf")
                grDevices::pdf(.tmp_pdf)

                on.exit({
                  .dev_after <- grDevices::dev.list()
                  .dev_after_ids <- if (is.null(.dev_after)) integer(0) else as.integer(.dev_after)

                  .extra <- setdiff(.dev_after_ids, .dev_before_ids)
                  if (length(.extra) > 0) {
                    for (d in rev(.extra)) {
                      grDevices::dev.set(which = d)
                      grDevices::dev.off()
                    }
                  }

                  if (base::file.exists(.tmp_pdf)) base::unlink(.tmp_pdf)
                }, add = TRUE)

                MEDBS_ks(data = Land, type = "l", SP = SPs[s], MS = MS, GSA = GSAs[g], Rt = Rt)
              }, error = function(e) NULL)
            )
          )
        },
      ...
      error = function(e) base::rep(list(NULL), nrow(.grid))
    )

    .res_mat <- vector("list", length(GSAs))
    for (g in 1:length(GSAs)) {
      .res_mat[[g]] <- vector("list", length(SPs))
    }
    for (i in seq_len(nrow(.grid))) {
      .res_mat[[.grid$g[[i]]]][[.grid$s[[i]]]] <- .res_list[[i]]
    }
  ...
  for (g in 1:length(GSAs)) {
    ...
    for (s in 1:length(SPs)) {
      cat(paste0("\n#### ", SPs[s], "\n"))
      res <- tryCatch(.ks_res_mat[[g]][[s]], error = function(e) NULL)
...
.len_ind_res_mat <- local({
# Save and restore the original future plan (cross-platform).
.old_plan <- future::plan()
on.exit(future::plan(.old_plan), add = TRUE)

future::plan(future::multisession)

# Build the grid of (g, s) combinations.
.grid <- base::expand.grid(
  g = 1:length(GSAs),
  s = 1:length(SPs)
)

# Run computations in parallel, keep failures as NULL (do not stop knitting).
.res_list <- tryCatch(
  furrr::future_map(
    seq_len(nrow(.grid)),
    function(i) {
...
# Rebuild results in a nested list structure: [[g]][[s]].
.res_mat <- vector("list", length(GSAs))
for (g in 1:length(GSAs)) {
  .res_mat[[g]] <- vector("list", length(SPs))
}
...
# sp_ALK= ALK[ALK$species %in% SPs & ALK$AREA %in% GSAs & ALK$COUNTRY %in% MS,]
...
```{r ALK_checks2, results = 'asis', fig.height=6, fig.width=9}

if (check_ALK & nrow(sp_ALK)>0) {
  cat("\n## Plots of mean length at age from the Age-Length Keys (ALK) data\n")
  ...
  for (g in 1:length(GSAs)) {
    ...
    for (s in 1:length(SPs)) {
      cat(paste0("\n#### ", SPs[s], "\n"))
      res <- suppressMessages(MEDBS_ALK_MLAA(ALK, SP = SPs[s], MS = MS, GSA = GSAs[g]))

       if (nrow(ALK)>0 & length(res)>0) {
          print(res)
          cat("\n\n\n")
              } else if (nrow(ALK)==0){
        cat(paste0("\n- No ALK data are available for the selected combination: ", SPs[s], " - ", MS, " - ", GSAs[g], ".\n"))
...
```{r ALK_checks3, results = 'asis', fig.height=6, fig.width=9}
if (check_ALK & nrow(sp_ALK)>0) {
  cat("\n## Check if the total number of hard structures is consistent with the sum of numbers by age in the Age-Length Keys (ALK) data\n")
  ...
  for (g in 1:length(GSAs)) {
    ...
    for (s in 1:length(SPs)) {
      cat(paste0("\n#### ", SPs[s], "\n"))
      res <- suppressMessages(MEDBS_ALK_NB(ALK, SP = SPs[s], MS = MS, GSA = GSAs[g]))

      if (nrow(ALK)>0 & nrow(res)>0) {
      cat(paste0("\n Records for which the total number of hard structures is not consistent with the sum of numbers by age from ALK for ", SPs[s], " - ", MS, " - ", GSAs[g], " - ", "\n"))
           print(res %>% kable("html") %>% kable_styling(font_size = 12) %>% kable_classic(full_width = TRUE))
         cat("\n\n\n")
        }    else if (nrow(ALK)==0){
        cat(paste0("\n- No ALK data are available for the selected combination: ", SPs[s], " - ", MS, " - ", GSAs[g], ".\n"))
      } else if (nrow(ALK)>0 & nrow(res)==0)
      {
       cat(paste0("\n- All records are consistent for the selected combination: ", SPs[s], " - ", MS, " - ", GSAs[g], ".\n"))   
      }
