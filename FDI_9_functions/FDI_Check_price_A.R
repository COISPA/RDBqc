...
FDI_ts_tableA <- function(data, MS, SP="COMBINED", GSA="COMBINED", verbose=TRUE) {
    ...
    if (nrow(data)==0) {
        ...
        suppressMessages(data1<-data%>%
                             drop_na(year, sub_region, country_code, species)%>%
                             select(year, sub_region, country_code, species, totwghtlandg, totvallandg)%>%
                             filter(sub_region%in%GSA & country_code%in%MS & species%in%SP)%>%
                             group_by(year, sub_region, country_code, species)%>%
        ...
        # Summary table of records by country and species
            ...
            suppressMessages(data2<-data%>%
                                 drop_na(year, country_code, species)%>%
                                 select(year, country_code, species, totwghtlandg, totvallandg)%>%
                                 filter(country_code%in%MS & species%in%SP)%>%
                                 group_by(year, country_code, species)%>%
                                 summarise(totwghtlandg=sum(as.numeric(totwghtlandg)),
                                           totvallandg=sum(as.numeric(totvallandg)),
                                           mean_price=totwghtlandg*1000/totvallandg))
            ...
            {plot_list[[i]]=
                ...
                ggplot(data=data3[data3$species==i,], aes(x =year , y=mean_price, col=sub_region, linetype=country_code ))+
                    geom_point()+
                    geom_line()+
