...
#' @export
#' @examples FDI_cross_checks_IG(data1=fdi_j_capacity, data2=fdi_g_effort)
...
FDI_cross_checks_JG <- function(data1, data2) {
    ...
    suppressMessages(data1 <- data1 %>%
                         group_by(country_code, year, sub_region,vessel_length ) %>%
                        summarize(total_vessels_tab_J=sum(as.numeric(total_vessels))))

    suppressMessages(data2 <- data2 %>%
                        group_by(country_code, year, sub_region,vessel_length ) %>%
                        summarize(total_vessels_tab_G=sum(as.numeric(total_vessels))))
