...
FDI_ts_tableJ <- function(data, MS, GSA,  vessel_len="COMBINED", fishtech="COMBINED", verbose=TRUE) {
    ...
    if (nrow(data)==0) {
        ...
        suppressMessages(data1<-data%>%
                             drop_na(year, sub_region, country_code, vessel_length, fishing_tech)%>%
                             select(year, sub_region, country_code, vessel_length, fishing_tech,
                                    total_trips,
                                    total_total_kW,
                                    total_GT,
                                    total_vessels
                             ...
                             group_by(year, sub_region, country_code, vessel_length, fishing_tech)%>%
                             summarise( total_trips=sum(!is.na(total_trips)) ,
                                        total_total_kW=sum(!is.na(total_total_kW)) ,
                                        total_GT=sum(!is.na(total_GT)),
                                        total_vessels=sum(!is.na(total_vessels))
                                        ))
        ...
        if (nrow(data1)==0) {
            ...
            suppressMessages(data3<-data2%>%
                                 ...
                                 filter(sub_region==GSA & country_code==MS )%>%
                                 group_by(year, sub_region, country_code, vessel_length, fishing_tech)%>%
                                 summarise(total_trips=sum(as.numeric(total_trips)),
                                           total_total_kW=sum(as.numeric(total_total_kW)),
                                           total_GT=sum(as.numeric(total_GT)),
                                           total_vessels=sum(as.numeric(total_vessels))
                                           ))
