...
```{r FDI_cov_tableA}
FDI_cov_tableA(data=fdi_a_catch, SP="MUT", MS="PSP",fishtech=unique(fdi_a_catch$fishing_tech), GSA="GSA99")
```


### Check of species value

The function finds an average price per species and year and compare it to the average price calculated per country.

```{r FFDI_species_price_table_A}
FDI_species_price(data=fdi_a_catch, MS="PSP",SP=c("MUT"), make_plot=TRUE, verbose = TRUE)
```
