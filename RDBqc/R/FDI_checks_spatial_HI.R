...
FDI_checks_spatial_HI <- function(data, MS, verbose=FALSE) {
    ...
    if (nrow(data)>0) {
        ...
        if (verbose) {message(paste(length(id_nk),"record/s with 'NK' values in 'sub_region' field"))}

        id_na <- which(is.na(data$sub_region) | data$sub_region =="NA")

        if (verbose) {message(paste(length(id_na),"record/s with 'NA' values in 'sub_region' field"))}
