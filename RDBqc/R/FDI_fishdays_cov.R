...
#' @description The function checks the coverage comparison of totfishdays in FDI tables G and I
...
FDI_fishdays_cov <- function(dataG, dataI, MS, verbose = TRUE){
    ...
    if (is.null(gsasG)) {stop('No SUB_REGIONS existing in table G')}
    if (is.null(yrsG)) {stop('No YEARS existing in table G')}
    if (is.null(gsasI)) {stop('No SUB_REGIONS existing in table I')}
    if (is.null(yrsI)) {stop('No YEARS existing in table I')}
    ...
    if (verbose) {
        if (length(naG1)!=0) {message(paste('Found NAs in SUB_REGIONS in', length(naG1), 'rows in table G' )) }
        if (length(naG2)!=0) {message(paste('Found NAs in Years in', length(naG2), 'rows, in table G' )) }
        if (length(naI1)!=0) {message(paste('Found NAs in SUB_REGIONS in', length(naI1), 'rows in table I' )) }
        if (length(naI2)!=0) {message(paste('Found NAs in Years in', length(naI2), 'rows, in table I' )) }
    }
    ...
    colnames(cov) <- c('year','country', 'gsa', 'Total fish days table G', 'Total fish days table I',
                       '% diff')
