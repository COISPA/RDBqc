...
FDI_fishdays_cov <- function (dataG, dataI, MS, verbose = TRUE)
{
  ...
  if (MS %in% mslistG & MS %in% mslistI) {
    if (verbose) {
      print(paste("Coverage of total fish days for", MS,
                  "data in tables G and I", sep = " "))
    }
    ...
    if (verbose) {
      if (length(naG1) != 0) {
        message(paste("Found NAs in SUB_REGIONS in",
                      length(naG1), "rows in table G"))
      }
      if (length(naG2) != 0) {
        message(paste("Found NAs in Years in", length(naG2),
                      "rows, in table G"))
      }
      if (length(naI1) != 0) {
        message(paste("Found NAs in SUB_REGIONS in",
                      length(naI1), "rows in table I"))
      }
      if (length(naI2) != 0) {
        message(paste("Found NAs in Years in", length(naI2),
                      "rows, in table I"))
      }
    }
    ...
    covG <- aggregate(list(fishdaysG = dataG$totfishdays),
                      by = list(year = dataG$year, country = dataG$country,
                                gsa = dataG$sub_region), FUN = sum, na.rm = T)
    covI <- aggregate(list(fishdaysI = dataI$totfishdays),
                      by = list(year = dataI$year, country = dataI$country,
                                gsa = dataI$sub_region), FUN = sum, na.rm = T)

    # merge tables by year and gsa
    cov <- merge(covG, covI, all.x = T, all.Y = T)
    cov$diff1 <- label_percent(1, accuracy = 0.01)(((cov$fishdaysI -
                                                       cov$fishdaysG)/cov$fishdaysG) * 100)
    colnames(cov) <- c("year", "country", "gsa", "Total fish days table G",
                       "Total fish days table I", "% diff")
