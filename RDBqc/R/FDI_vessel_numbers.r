...
#' @description The function cross-checks the number of vessels in table J in comparison of the number reported in table G.
#' @return The function returns a list of data frames. The first element reports the number of vessel in table J in comparison to table G and the relative difference percentage.
#' @export
...
FDI_vessel_numbers <- function(dataJ, dataG, MS, verbose = TRUE){
    ...
    if (is.null(yrs1)) {stop('No YEARS existing in table J')}
    if (is.null(yrs2)) {stop('No YEARS existing in table G')}
    ...
    if (verbose) {
        if (length(na1)!=0) {message(paste('Found NAs in Years in', length(na1), 'rows, in table J' )) }
        if (length(na2)!=0) {message(paste('Found NAs in Years in', length(na2), 'rows, in table G' )) }
    }
