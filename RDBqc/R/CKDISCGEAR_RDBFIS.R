...
#' @examples \donttest{# Check_Tot_Disc_gear(MEDBS,FDI,"ITA","GSA10","HKE",MEDBSSP)}
#' # The function works by one country, subarea and species each.
#' # It is not possible assign more country, subarea or species.

Check_Tot_Disc_gear <- function(data,data1,MS,GSA,SP,MEDBSSP,verbose=TRUE,OUT=FALSE){

  AREA <- COUNTRY <- DATA_CALL <- DISCARDS <- GEAR <- GEAR_TYPE <- LANDINGS <- SPECIES <- SUB_REGION <- YEAR <- NULL
  ...
  compland <- list()
  if (nrow(data)>0){
    compland[[1]] <- setdiff(paste0(data$YEAR,"_",data$GEAR),
                                   paste0(data1$YEAR,"_",data1$GEAR_TYPE))

  names(compland)[[1]] <- paste("TABLE_DISCARDS_GEAR_IN_MEDBS_NOT_IN_FDI",MS,GSA,SP,sep="_")

               if(OUT%in%TRUE){
                 WD <- getwd()
                 suppressWarnings(dir.create(paste0(WD,"/OUTPUT/CSV"),recursive = T))
    write.csv(setdiff(paste0(data$YEAR,"_",data$GEAR),
                      paste0(data1$YEAR,"_",data1$GEAR_TYPE)
  ...
  if (nrow(data1)>0){
    compland[[2]] <- setdiff(paste0(data1$YEAR,"_",data1$GEAR_TYPE),
                                    paste0(data$YEAR,"_",data$GEAR))
    names(compland)[[2]] <- paste("TABLE_DISCARDS_GEAR_IN_FDI_NOT_IN_MEDBS",MS,GSA,SP,sep="_")


if(OUT%in%TRUE){
  WD <- getwd()
  suppressWarnings(dir.create(paste0(WD,"/OUTPUT/CSV"),recursive = T))
    write.csv(setdiff(paste0(data1$YEAR,"_",data1$GEAR_TYPE),
                      paste0(data$YEAR,"_",data$GEAR)
  ...
  if(nrow(data)>0){
  data <- suppressMessages(data %>% group_by(YEAR,COUNTRY,AREA,SPECIES,GEAR) %>% summarize(TOT=sum(DISCARDS,na.rm=T)))
  ...
  if(nrow(data1)>0){
  data1 <- suppressMessages(data1 %>% group_by(YEAR,COUNTRY,SUB_REGION,SPECIES,GEAR_TYPE) %>% summarize(TOT=sum(DISCARDS,na.rm=T)))
  ...
  if(nrow(rbind(db,db1))>0){
    ...
    names(compland)[[3]] <- paste(paste("TABLE_DISCARDS_GEAR",MS,GSA,SP,sep="_"))
    counter <- 4

    colorset = c('FDI'='red','MEDBS'='blue')
    for (i in union(db$GEAR,db1$GEAR)){
      if(nrow(rbind(db,db1)[rbind(db,db1)$GEAR%in%i,])>0){
        compland[[counter]] <- suppressMessages(plot_grid(ggplot(rbind(db,db1)[rbind(db,db1)$GEAR%in%i,],aes(x=YEAR,y=DATA_CALL,col=DATA_CALL))+geom_point()+ylab("")+xlab("")+
