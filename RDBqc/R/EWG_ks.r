...
EWG_ks <- function (data,type,MS,GSA,SP,Rt=1) {
    ...
    if (FALSE) {
        ...
        EWG_ks(data=landing, type="l",MS="ITA",GSA="18", SP="NEP",Rt=1)
    ...
    if (type == "l") {
        ...
        NUMBERb <- LFL %>% group_by(year,gear,fishery) %>%
            summarize(total_number = sum(value))

        MEANdb <- LFL %>% group_by(year,gear,fishery) %>%
        ...
        SDdb <- LFL_mutate %>% group_by(year,gear,fishery) %>%
            summarize(sd_size = sqrt(sum(fsquare)/(sum(value)-1)))

        MINdb <- LFL %>% group_by(year,gear,fishery) %>%
            filter(value > 0, na.rm = TRUE) %>%
            summarize(min_size = min(start_length,na.rm = T))

        MAXdb <- LFL %>% group_by(year,gear,fishery) %>%
            filter(value > 0, na.rm = TRUE) %>%
    ...
    ###########
        ...
        LFLandings_tot <- LFLandings %>% group_by(year,gear,fishery) %>% summarize(tot=sum(value))
