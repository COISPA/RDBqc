...
#' @export Check_Tot_Land2
#' @importFrom dplyr summarize group_by
#' @importFrom data.table fread
#' @import ggplot2
#' @importFrom cowplot plot_grid
#' @description The function compares the landings in weight values reported in the MEDBS landings by length table with the ones reported in the FDI Table A catch table and AER data (https://stecf.jrc.ec.europa.eu/reports/economic). The analysis is done at MS, GSA and species level
...
Check_Tot_Land2 <- function(data, data1, data2, MS, GSA, SP, MEDBSSP, verbose = TRUE, OUT = FALSE) {
  ...
  if (id %in% c(unique(paste0(data$COUNTRY, "_", data$AREA, "_", data$SPECIES))) |
    ...
    data2 <- data2[data2$COUNTRY_CODE %in% MS & data2$SUB_REG %in% GSA & data2$SPECIES_CODE %in% SP, ]
    if (nrow(data2) > 0) {
      suppressMessages(data2 <- data2 %>% group_by(YEAR, COUNTRY_CODE, SUB_REG, SPECIES_CODE) %>% summarize(TOT = sum(VALUE, na.rm = T)))
      db2 <- data.frame("YEAR" = data2$YEAR, "COUNTRY" = data2$COUNTRY_CODE, "GSA" = data2$SUB_REG, "SPECIES" = data2$SPECIES_CODE, "LANDINGS" = data2$TOT, "DATA_CALL" = "AER")
    } else {
      db2 <- data.frame(
        "YEAR" = integer(),
        "COUNTRY" = character(),
