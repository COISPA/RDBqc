...
check_EF_FDI_G <- function(data, verbose = TRUE)
{
  if (FALSE){
    ...
    verbose = TRUE
    check_EF_FDI_G(data, verbose = TRUE)
  }
  ...
  results2 <- list(NA_finder_col1, NA_finder_col2, NA_finder_col3,
                   ...
                   NA_finder_col20, NA_finder_col21, NA_finder_col22, NA_finder_col23,
                   NA_finder_col24)
  ...
  if (verbose) {
    if (length(NA_finder_col21) == 0) {
      message(paste("no NA in the", colnames(data)[21],
                    "column"))
    }
    else {
      message(paste("There are ", length(NA_finder_col21),
                    " NA in ", colnames(data)[21]))
    }
  }
  if (verbose) {
    if (length(NA_finder_col22) == 0) {
      message(paste("no NA in the", colnames(data)[22],
                    "column"))
    }
    else {
      message(paste("There are ", length(NA_finder_col22),
                    " NA in ", colnames(data)[22]))
    }
  }
  if (verbose) {
    if (length(NA_finder_col23) == 0) {
      message(paste("no NA in the", colnames(data)[23],
                    "column"))
    }
    else {
      message(paste("There are ", length(NA_finder_col23),
                    " NA in ", colnames(data)[23]))
    }
  }
  if (verbose) {
    if (length(NA_finder_col24) == 0) {
      message(paste("no NA in the", colnames(data)[24],
                    "column"))
    }
    else {
      message(paste("There are ", length(NA_finder_col24),
                    " NA in ", colnames(data)[24]))
    }
  }
