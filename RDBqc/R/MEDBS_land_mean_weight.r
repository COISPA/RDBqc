...
MEDBS_land_mean_weight <- function(data, SP, MS, GSA, verbose = TRUE) {
  ...
  if (nrow(land) < 2) {
    ...
    names(ck_nbl)
    suppressMessages(Wg <- ck_nbl %>% group_by(year, quarter, gear, fishery) %>% summarize(totW = sum(landings) * 1000000))
    suppressMessages(No <- ck_nbl %>% group_by(year, quarter, gear, fishery) %>% summarize(totN = sum(sum) * 1000))
