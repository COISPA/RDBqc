...
#' @examples MEDBS_plot_discard_ts(
...
MEDBS_plot_discard_ts <- function(data, SP, MS, GSA, by = "year") {
  year <- gear <- discards <- quarter <- tot <- NULL
  ...
  if (nrow(disc) > 0) {
    ...
    gear_tmp <- data.frame("year" = rep(min(disc_tmp$year):max(disc_tmp$year)), "gear" = rep(unique(disc_tmp$gear), each = (max(disc_tmp$year) - min(disc_tmp$year) + 1)))
    suppressMessages(discqrt_tmp <- as.data.frame(disc %>% group_by(year, quarter) %>% summarize(tot = sum(discards))))
    quarter_tmp <- data.frame("year" = rep(min(disc_tmp$year):max(disc_tmp$year)), "quarter" = rep(unique(discqrt_tmp$quarter), each = (max(disc_tmp$year) - min(disc_tmp$year) + 1)))
