...
Check_Tot_Land_gear_Q <- function(data, data1, MS, GSA, SP, MEDBSSP, verbose = TRUE, OUT = FALSE) {

  AREA <- COUNTRY <- DATA_CALL <- GEAR <- GEAR_TYPE <- LANDINGS <- QUARTER <- SPECIES <- SUB_REGION <- TOTWGHTLANDG <- YEAR <- NULL
  ...
  if (id %in% c(unique(paste0(data$COUNTRY, "_", data$AREA, "_", data$SPECIES))) | id %in% c(unique(paste0(
    ...
    if (nrow(data) > 0) {
      ...
      names(compland)[[1]] <- paste("TABLE_LANDINGS_GEAR_QUARTER_IN_MEDBS_NOT_IN_FDI", MS, GSA, SP, sep = "_")
    ...
    if (nrow(data1) > 0) {
      ...
      names(compland)[[2]] <- paste("TABLE_LANDINGS_GEAR_QUARTER_IN_FDI_NOT_IN_MEDBS", MS, GSA, SP, sep = "_")
    ...
    if (nrow(data) > 0) {
      data <- suppressMessages(data %>% group_by(YEAR, COUNTRY, AREA, SPECIES, GEAR, QUARTER) %>% summarize(TOT = sum(LANDINGS, na.rm = T)))
      db <- data.frame(
    ...
    if (nrow(data1) > 0) {
      data1 <- suppressMessages(data1 %>% group_by(YEAR, COUNTRY, SUB_REGION, SPECIES, GEAR_TYPE, QUARTER) %>% summarize(TOT = sum(TOTWGHTLANDG, na.rm = T)))
      db1 <- data.frame(
    ...
    if (nrow(rbind(db, db1)) > 0) {
      ...
      names(compland)[[3]] <- paste(paste("TABLE_LANDINGS_GEAR_QUARTER", MS, GSA, SP, sep = "_"))
    }
    counter <- 4
    ...
    if (OUT %in% TRUE) {
      WD <- getwd()
      suppressWarnings(dir.create(paste0(WD, "/OUTPUT/CSV"), recursive = T))
      write.csv(rbind(db, db1), paste0(WD, "/OUTPUT/CSV/Landings gear quarter comparison for ", MS, "_", GSA, "_", SP, ".csv"), row.names = F)
    }
    ...
    for (i in union(db$GEAR, db1$GEAR)) {
      for (j in union(db$QUARTER, db1$QUARTER)) {
        if (nrow(rbind(db, db1)[rbind(db, db1)$GEAR %in% i & rbind(db, db1)$QUARTER %in% j, ]) > 0) {
          # ggsave(paste0("../OUT/PLOT/Landings gear ",i," and quarter ",j," comparison for ",MS,"_",GSA,"_",SP,".jpeg"),  units="in", width=8, height=4, dpi=300,
          compland[[counter]] <- suppressMessages(plot_grid(
            ggplot(rbind(db, db1)[rbind(db, db1)$GEAR %in% i & rbind(db, db1)$QUARTER %in% j, ], aes(x = YEAR, y = DATA_CALL, col = DATA_CALL)) +
              geom_point() +
              ylab("") +
              xlab("") +
              scale_color_manual(values = colorset) +
              scale_x_continuous(breaks = seq(min(rbind(db, db1)[rbind(db, db1)$GEAR %in% i & rbind(db, db1)$QUARTER %in% j, ]$YEAR), max(rbind(db, db1)[rbind(db, db1)$GEAR %in% i & rbind(db, db1)$QUARTER %in% j, ]$YEAR), 1)) +
              theme_bw() +
              ggtitle(paste0("Time series available for ", MS, "_", GSA, "_", SP, "_", i, "_in quarter ", j)) +
              theme(legend.position = "none"),
            ggplot(rbind(db, db1)[rbind(db, db1)$GEAR %in% i & rbind(db, db1)$QUARTER %in% j, ], aes(x = YEAR, y = LANDINGS, col = DATA_CALL)) +
              geom_point() +
              geom_line() +
              ylab("Landings (t)") +
              scale_color_manual(values = colorset) +
          ...
          # )
          names(compland)[[counter]] <- paste("PLOT_LANDINGS_GEAR", i, "QUARTER", j, MS, GSA, SP, sep = "_")
          counter <- counter + 1
        } # else{print("")}
      }
    }
    if (OUT %in% TRUE) {
      ...
      for (i in union(db$GEAR, db1$GEAR)) {
        for (j in union(db$QUARTER, db1$QUARTER)) {
          if (nrow(rbind(db, db1)[rbind(db, db1)$GEAR %in% i & rbind(db, db1)$QUARTER %in% j, ]) > 0) {
            ggsave(paste0(WD, "/OUTPUT/PLOT/Landings gear ", i, " and quarter ", j, " comparison for ", MS, "_", GSA, "_", SP, ".jpeg"),
