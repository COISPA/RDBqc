  ...
  if (type == "l") {
    ...
    if (nrow(land) < 2) {
      ...
      if (sum(p$Sum) > 0) {
        ...
        LFL <- suppressMessages(ldat %>% group_by(
          year, gear,
          fishery, start_length
        ...
        suppressMessages(MEANdb <- LFL %>% group_by(
          year,
          gear, fishery
        ...
        suppressMessages(SDdb <- LFL_mutate %>% group_by(
          year,
          gear, fishery
        ...
        suppressMessages(MAXdb <- LFL %>% group_by(
          year,
          gear, fishery
        ) %>% filter(value > 0, na.rm = TRUE) %>%
          summarize(max_size = max(start_length, na.rm = T)))
        suppressMessages(final_DB <- left_join(MINdb, MEANdb))
        suppressMessages(final_DB <- inner_join(
          final_DB,
          MAXdb
        ))
        suppressMessages(final_DB <- left_join(
          final_DB,
          SDdb
        ))
        ...
        dbland <- setNames(NUMBERb, c(
          "year", "gear",
          "fishery", "total_number_landed"
        ))
        ...
        suppressMessages(LFLandingsDB <- left_join(
          final_DB,
          db27
        ))
        result <- LFLandingsDB
  ...
  if (type == "d") {
    ...
    if (nrow(disc) < 2) {
      ...
      if (sum(p$Sum) > 0) {
        ...
        LFD <- suppressMessages(ddat %>% group_by(
          year,
          gear, fishery, start_length
        ) %>% summarise(value = sum(value)))
        names(LFD) <- c(
          "year", "gear", "fishery",
          "start_length", "value"
        ...
        LFDiscards <- LFL
        suppressMessages(db <- left_join(NUMBERb, db))
        dbdisc <- setNames(NUMBERb, c(
          "year", "gear",
          "fishery", "total_number_discarded"
        ))
        ...
        db27 <- do.call(rbind, tmp1)
        suppressMessages(LFDiscardsDB <- left_join(
          final_DB,
          db27
        ))
        result <- LFDiscardsDB
