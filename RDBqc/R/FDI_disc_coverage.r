  ...
  if (MS %in% mslist) {
    ...
    if (nrow(data1) > 0) {
      ...
      if (verbose) {
        ...
        if (length(na2) != 0) {
          message(paste("Found NAs in Years in", length(na2),
                        "rows"))
        }
      }
      if (any(is.na(data1$discards))) {
        data1$discards[which(is.na(data1$discards))] <- "NK"
        if (verbose) {
          message("Warning: Found NAs in DISCARDS values. Substituted NAs with NKs")
        }
      }
      ...
      discov <- aggregate(list(landings = data1$totwghtlandg),
                          by = list(year = data1$year, gsa = data1$sub_region,
                                    disccat = data1$DISCcat), FUN = sum, na.rm = T)
      TL <- aggregate(list(landings = data1$totwghtlandg),
                      by = list(year = data1$year, gsa = data1$sub_region),
                      FUN = sum, na.rm = T)
      ...
      colnames(dd3) <- c("year", "gsa", "Total_lands",
                         "Lands_(disc > 0)", "% Lands_(disc >0)", "Lands_(disc = 0)",
